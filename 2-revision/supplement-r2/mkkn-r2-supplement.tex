...
P(\theta | X,y,z) &\propto & P(X,y,z | \theta) P(\theta) \\
  &=&  \left[ P(y, z | \phi,\psi ) P(X | z, \mu, \sigma) \right]  \left[ P(\mu,\sigma ) P(\phi, \psi) \right]\\
  &=&  \left[ P(y, z | \phi,\psi ) P(\phi,  \psi) \right] \left[  P(X | z, \mu, \sigma)  P(\mu,\sigma ) \right]\\
 &\propto& P(\phi,\psi | y, z) \, P(\mu, \sigma | X, z)  .
\end{eqnarray*}
Thus 
\[ P(\theta | X,y,z) &\propto P(\phi,\psi | y, z) P(\mu, \sigma | X, z) \],
and it follows by integration over the parameter space that
...
$X_{g, s}^I | \sigma_g, q_g^I \sim NB(\sigma_{g}, q_g^I)$ with density  $P(X_{g,s}^I | \sigma_g, q_g^I ) = {X_{g,s}^I + \sigma_g - 1 \choose X_{g,s}^I }(1 - q_g^I)^{X_{g,s}^I} (q_g^I)^{\sigma_g}$
...
clustering (\cite{DPpack})  on  a simulated data set. 
...
To demonstrate the change of PDD over different $K$, we present an example using data set
...
\caption{PDD change under different number of subtypes $K$ for data set DEC-EC (GSE75748). We select $K = 4$, which also stabilize the PDD.}
\label{fig:rwk}
\end{figure}

\begin{figure}[h]
\includegraphics[width = 0.7\textwidth]{Figs/rw.pdf}
\caption{PDD under $K = 5$ vs. $K = 6$ for data set DEC-EC (GSE75748). PDD without randomization (left) vs. PDD with randomization (right). \texttt{scDDboost} gained robustness through averaging over 
...
\includegraphics[width = 0.5\textwidth]{Figs/breakFDR.pdf}
 \caption{We may lose FDR control when $K$ is too large.  Calculations here 
 use data set EMTAB2805null (Table S2) and target 5\% FDR, which is not controlled when $K>6$
But by the validity score method we would choose $K$ much smaller, as that score stabilizes when $K\geq3$. 
  \label{fig:lfdr}}
\end{figure}
...
\begin{figure}[h!]
  ...
  This measures the degree of difficulty of the 12 simulation scenarios, with those on the left presenting
a difficult challenge to identify altered genes.}
  \label{fig:t-s}
\end{figure}
...
\begin{figure}[h!]
  \includegraphics[ width = 0.9\textwidth]{Figs/ROC.pdf}
  \caption{ROC curve in 12 simulation settings. In each setting, TPR and FPR are averaged over ten replicates, generally \texttt{scDDboost} performs better than other methods, orange dashed line is the identity,
and all panels show the unit square.}
  \label{fig:roc}
\end{figure}
...
In this section, we provide details of the empirical data sets and also demonstrate an empirical relationship
 between \texttt{scDDboost} scores to the Wasserstein distance between two samples.


\noindent
{\bf Data sets} Details for the data sets used in the empirical studies with the estimated number of subtypes $K$ are shown in Supplementary Table \ref{table:1}.
...
\end{tabular}
\caption{Data sets used for empirical study}
\label{table:1}
\end{table}

For the first 11 data sets in Supplementary Table \ref{table:1} we use all the cells within that condition under same batch.
The last one is the largest data set we explored containing 3589 cells and comparing neoplastic cells (1091 cells) vs non-neoplastic cells (2498 cells).
...
We observe an association empirically between 
distributional change measurements by \texttt{scDDboost} and Wasserstein distance between samples
...
Genes associated with boxes from left to right having $P(ED_g|X,y)$ range from 0 - 0.2, 0.2 - 0.4, 0.4 - 0.6, 0.6 - 0.8, 0.8 - 1. This is taken from the stem-cell data: GSE64016}.
 \label{fig:wad}
\end{figure}
...
Data sets used for generating the null cases  are shown in Supplementary Table \ref{table:2}.
...
\hline
 Data set & Conditions & Number of cells & Organism \\
\hline
\hline
GSE63818null & 7 week gestation  & 40 &mouse \\
\hline
GSE75748null & DEC & 64 & human\\
\hline
GSE94383null & T0 & 186 & human \\
\hline
GSE48968-GPL13112null & BMDC (2h LPS stimulation) & 96 & mouse \\
%\hline
% GSE60749-GPL13112null & v6.5 mouse embryonic stem cells, culture conditions: 2i+LIF & 90 & mouse \\
 \hline
 GSE74596null & NKT1 & 46 & mouse\\
 \hline
 EMTAB2805null & G1 & 96 & mouse\\
 \hline
GSE71585-GPL13112null &Gad2tdTpositive  & 80 & mouse \\
\hline
GSE64016null & G1 & 91 & human \\
\hline
GSE79102null & patient1 & 51 & human\\
\hline
\end{tabular}
\caption{Data sets used for null cases. Cells arise from one  biological condition in each case.  We 
split randomly in half multiple times for negative control calculations, }
\label{table:2}
\end{table}
...
%D3E\citep{ref:d3e} is a distributional method that can identify bursting parameters of transcripts. Rate of promoter activation, rate of promoter inactivation and the rate of transcription when the promoter is in the active state are estimated by D3E.  We investigate DD genes identified by scDDboost and their change of those three parameters on data set GSE71585\\
...
negative binomial (NB) assumption in three data sets (GSE64016, EMTAB2805 and GSE45719).
Given the estimated subtype label, we test the goodness of fit for NB model within subtype clusters.
We investigated the distribution of the p-values 
and observed that some genes show evidence against the NB assumption.
Let $\mathcal{N}_{0.05}$ be the set of genes having  FDR adjusted goodness-of-fit p-value (by BH) 
 less than 0.05 on at least one subtype. We call these the non-NB genes.  Relating this to the DD calls,
let $\mathcal{D}_{0.05}$ be the set of genes identified as DD genes by our method at 0.05 local FDR and
let $\mathcal{U}_{0.05}$ be the union of genes identified by other methods (t-test, MAST, DESeq2, scDD) 
with similar FDR control.
Supplementary Table \ref{tab:nb} demonstrates that among the non-NB genes, most called DD by 
\texttt{scDDboost} 
 are also confirmed by other tests, suggesting that model misspecification may not often lead to erroneous inferences.
For the stem-cell data GSE64016, which we used in Figure 1 in the main paper, 
...
%%     \cmidrule(r){1-5} % Partial rule. (r) trims the line a little bit on the right; (l) & (lr) also possible
     ...
     $|\mathcal{D}_{0.05} \cap \mathcal{N}_{0.05}| $ & $|\mathcal{D}_{0.05} \cap \mathcal{N}_{0.05} \cap \mathcal{U}_{0.05}|$ & $\#$ genes  \\
     \midrule
     ...
     \bottomrule
 \end{tabular}
 \caption{Number of DD genes in relation to the genes 
 that fail the NB goodness-of-fit test.}
 \label{tab:nb}
 \end{center}
\end{table}


Supplementary Figure \ref{fig:nb} presents the cumulative distribution of goodness-of-fit test p-values under each subtype, and also indicates relatively little lack of fit in these cases.
\begin{figure}[h!]
  \includegraphics[width=0.8\linewidth]{Figs/nbfit.pdf}
  \caption{Cumulative distribution functions for  p-values of goodness of fit for NB distribution at two data sets (left is EMTAB2805, right is GSE45719), with different colors corresponding to different subtypes}
  \label{fig:nb}
\end{figure}
...
Group differences are evident in genes that show non-NB behavior
and that are uniquely identified by \texttt{scDDboost} as being DD (Supplementary Figure~\ref{fig:s11}).   
\begin{figure}[h]
\includegraphics[width = 0.9\textwidth]{Figs/NBgene.pdf}
\caption{Data set EMTAB2805, violin plot of the 57 genes having evidence against negative binomial distribution assumption and uniquely called DD by \texttt{scDDboost}}
\label{fig:s11}
\end{figure}
...
\sigma^2 = \dots = \sigma^K,$$ the test statistic is $2(l_1 - l_0)$, where $l_1$ is the log likelihood maximized over $r$ and $p$ given the data. $l_0$ is the log likelihood maximized under the $H_0$ constraint. We optimize numerically
and consider the LRT to have an approximate chi-square distribution on the null.  We use simulation to check the validity of the chi-square approximation. We simulate 500 random variables that are NB(3,0.5) distributed and another 500 random variables with distribution NB(3,0.7). The test statistic and chi-square-based p-values are recorded, and this calculation 
is  repeated 100 times (Supplementary Figure~\ref{fig:lrt}). 
The LRT with chi-square p-value is approximately uniform on the null, and so
\begin{figure}[h!]
  \includegraphics[ width = 0.7\textwidth]{Figs/lrt.pdf}
  \caption{Cumulative distribution for chi-square p-values of LRT in a null case shows a slight inflation 
of type-I error for the goodness-of-fit test}
  \label{fig:lrt}
\end{figure}
we use it to flag genes that may violate the constant-shape assumption.
Let $\mathcal{R}_{0.05}$ be the set of genes having BH-adjusted LRT p-value less than 0.05, and 
let $\mathcal{U}_{0.05}$ be the union genes identified by t-test, DESeq2, MAST or scDD as differential between
the two groups.
Table \ref{tab:cs} demonstrates that most genes identified by \texttt{scDDboost} as DD are also confirmed by other tests. 
\begin{table}[htbp]
  \begin{center}
  ...
  \begin{tabular}{@{} lcccr @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05}| $ & $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05} \cap \mathcal{U}_{0.05}|$ & $\#$ genes   \\
      \midrule
      ...
      \bottomrule
  \end{tabular}
  \caption{Number of DD calls in genes that violate the constant-shape test}
  \label{tab:cs}
  \end{center}
\end{table}
Though some of the non-constant shape genes  are  uniquely called as DD by \texttt{scDDboost}, 
Figure~\ref{fig:NCS} shows that they tend to have smaller DD p-values by other methods, as
compared to other genes, suggesting it is plausible that the DD detection is not a false call.
\begin{figure}[h!]
  \includegraphics[ width = 0.8\textwidth]{Figs/Fucci-NCS.pdf}
  \caption{Boxplots of FDR adjusted p-values by other methods, separated by non-constant shape genes uniquely identified
by scDDboost (left) compared to the whole genome (right).  }
  \label{fig:NCS}
\end{figure}
We also see this phenomonon in the \verb+splatter+ simulation.
 We ran the LRT on the synthetic data. Roughly 1000 genes 
violate the constant-shape assumption in each case. 
Among those 1000 genes, \texttt{scDDboost} on averaage identified 50 DD genes and on average 49 genes are true DD genes.  Thus lack of constant shape does not invalidate the DD inferences in the
cases analyzed. 


\subsubsection*{Clustering distance}

How sensitive are \texttt{scDDboost} results to the distance used by K-medoids clustering?  To investigate, we
applied the SC3 distance in place of the default in several  numerical experiments.
...
We also ran \texttt{scDDboost} using SC3 in the simulation study (compare to Figures 5, main). 
...
\begin{figure}[h!]
  ...
  \caption{FDR by  simulation, when using \texttt{scDDboost} with SC3 clustering}
  \label{fig:sc3fdr}
\end{figure}

 
We redeployed \texttt{scDDboost} in several data sets 
using \texttt{SC3} distance in place of the default distance.
Let $p_o$ be the proportion of commonly identified genes among the top 1000 and $n_{DD}$ be the number of DD genes. 
The distance has an effect on the final list of DD genes, but there is broad agreement 
in those identified (Supplementary Table~\ref{tab:dist}).  
%he default distance tends to post fewer genes than  \texttt{SC3}, though what is found by the default
%is mostly found by \texttt{SC3} 
\begin{table}[htbp]
  \begin{center}
  %\topcaption{Table captions are better up top} % requires the topcapt package
  \begin{tabular}{@{} lccccr @{}} % Column formatting, @{} suppresses leading/trailing space
      \toprule
      %\multicolumn{2}{c}{Item} \\
      \cmidrule(r){1-5} % Partial rule. (r) trims the line a little bit on the right; (l) & (lr) also possible
      Data Set   & 
      ...
      \midrule
      EMTAB2805   & 80\% & 3334 & 3725 & 2913\\
      GSE45719    & 75\% & 4851 & 7591 & 4805\\
      GSE64016  & 80\% & 3395& 665 & 529\\
      \bottomrule
  \end{tabular}
  \caption{Relation of findings from two different distance methods in two data sets (rows). $n_{DD}$ is the number found DD at 5\% FDR.
Notice that $p_o$ for GSE64016 is just $529 / 665$}
  \label{tab:dist}
  \end{center}
\end{table}

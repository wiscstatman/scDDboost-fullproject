...
\raggedright\hspace*{\algorithmicindent} \textbf{Input}: Expression data matrix $X$ and condition label vector  $y$ \\
\hspace*{\algorithmicindent} \textbf{Output}: posterior probabilities of genes being differential distributed
\begin{algorithmic}[1]
\Procedure{scDDboost0}{$X,y$}
\State partition cells using distances between columns of $X$
\State $z^1, z^2\gets$ partition of cells and $y$ 
...
\hspace*{\algorithmicindent} \textbf{Output}: clustering of cells $cl$
\begin{algorithmic}[1]
...
\State $cl\gets K\text{-medoids on }\hat{D}$
...
\For {$i = 0, i < T$}  
\State $cl\gets$ Random-c($D, K$)
\State $z^1, z^2\gets cl$ and $y$ 
\State $P(M_{g,\pi} | X)\gets \text{EBSeq}$\cite{ref:Leng}
\State $P(A_\pi | X,y)\gets \text{Empirical Bayes}$
\State $P(\text{ED}_g | X, y)\gets \underset{\pi}{\sum}P(M_{g,\pi} | X)P(A_\pi | z^1, z^2)$
\State $P(\text{DD}_g | X, y) \gets 1 - P(\text{ED}_g | X, y)$

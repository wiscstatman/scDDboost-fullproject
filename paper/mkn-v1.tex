...
\begin{theorem} assuming $\beta_b = \Sigma_{k\in b} \alpha_k$, and $\alpha_k = 1, \forall k$, let $n = min(n_1, n_2)$ be the smaller one of number of cells of two conditions and $\frac{n_1}{n_2} = C(C != 0)$  when $(\phi, \psi)\in \Omega\setminus N $ we have 
\begin{eqnarray*}
...
\end{eqnarray*}
\end{theorem}
\begin{theorem} when $(\phi, \psi)\in N$,  and  $S$ = $\{\pi$,  $(\phi, \psi) \in A_\pi\}$, we have 
\begin{eqnarray*}
    p(A_{\pi} | z^1, z^2) \xrightarrow[n\rightarrow\infty]{\text{a.s.}}\left\{
                \begin{array}{ll}
                 m(\pi) \quad \text{if }(\phi,\psi) \in A_\pi \text{ and } \pi \in S\\
                 0 \quad \text{otherwise}\\             
                \end{array}
              \right.
\end{eqnarray*}
...
\leq f(n, b)\leq\\
\beta_b - 2\text{ln}\sqrt{2\pi} + g(n, b)$$\\
...
and sum up 
$$\sum_{b\in\pi} g(n,b) = 2n\text{ln}2 - \sum_{b\in\pi}(\beta_b - \frac{1}{2})\text{ln}(\beta_b + t_b^1 + t_b^2 - 1) + O_p(1) +  O_p(\frac{1}{n})$$

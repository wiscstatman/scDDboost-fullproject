...
\begin{algorithmic}[1]
\Procedure{scDDboost}{$X_1,X_2$}
\State $Z\gets \text{identify subtypes of pooled cells} (X_1,X_2)$
\State $P(M_{g,\pi} | X_1, X_2)\gets \text{EBSeq}\cite{ref:Leng}$
\State $P(A_\pi | Z)\gets \text{Empirical Bayes via double dirichlet prior}$
...
Prior to any bayesian inference, we need to identify subtypes of cells. One advantage of our approach is that the posterior inference can be incorporate with different clustering methods.
...
After identification of subtypes, the following two steps use empirical Bayes to provide posterior inference on patterns of differential expression and aggregated proportions of subtypes, which are components of posterior probabilities for a gene being equivalent distributed. Combining those components, final posterior probabilities are obtained.
...
10\% DE genes in each subtype results in total 9067 DD genes and 8306 ED genes in the mixture of the 7 subtypes.
...
scDDboost identified most true DD genes, the reason is that mean expression shifts between conditions is not as significant as mean expression shifts between subtypes, which limits the power of MAST and DESeq2. Our approach and scDD considered mixture structure underlying the transcrips but scDD did not use the whole genome information to infer mixture components, which leads to inaccurate clustering at gene level and reduce the power. scDDboost could correctly identify the subtypes of cells and thus are more sensitive to the mean expression change among subtypes. We also compare roc curves of scDDboost, scDD, MAST and DESeq2. (figure 5)
\begin{figure}[h]
\vspace{-\parskip}
...
Since we are modeling gene transcript within each subtype as negative binomial distributed and we only test one parameter(mean) change among subtypes. In some situation, it could be insufficient for modeling the variability within subtype. Even though there is no mean expression change among subtypes but the distribution among subtypes changed, EBSeq would fail to detect the discrepancies between subtypes, thus reduce power of detecting DD genes.\\
...
\begin{figure}[h]
\includegraphics[width = \textwidth]{density_DEC_scdb_dd.pdf}

...
\begin{lemma}
If $\pi_2$ is not refinement of $\pi_1$ then $A_{\pi_1} \cap A_{\pi_2}$ is a lower dimensional simplex of $A_{\pi_2}$
\end{lemma}
\hfill\\
...
\end{figure}
\hfill\\
We look at the lines in Figure1, 
We lack posterior inference for $(\phi,\psi)$ along the purple line(Figure 1) except the black dot. The black dot is the simplex with finest partitions, $\phi_i = \psi_i, \forall i = 1,..,K$. The purple line is the intersection of two simplexes: rectangle with magenta boundary and rectangle with blue boundary. 
 
\begin{theorem}  Assuming  $\beta_b = \Sigma_{i\in b} \alpha_i$, and $\alpha_i = 1, \forall i$, then we have simplified $p(z^1, z^2 | A_\pi) = \prod_{b\in \pi}\frac{ \Gamma(\beta_b + t_b^1 + t_b^2)}{\Gamma(\beta_b + t_b^1)\Gamma(\beta_b + t_b^2)}\frac{\Gamma(n + 1)\Gamma(n+1)\Gamma(K)}{\Gamma(2n + K)}$,  let $n = min(n_1, n_2)$ be the smaller number of cells of two conditions and $n_1 = O(n_2)$, when parameter $(\phi, \psi)\in \Omega\setminus Q $ we have 
\begin{eqnarray*}
...
\end{eqnarray*}
\end{theorem}
If proportions of subtypes(parameter) does not satisfy partial intersection constraints, there is a unique subspace $A^*_\pi$ cover the parameter.  When numbers of cells of condition 1 and 2 go to infinity at same rate, we have the posterior probability of subspace $A_\pi$ that covers the parameter go to 1 almost surely.
\begin{theorem} Following the setting in theorem 2, when parameter $(\phi, \psi)\in Q$,  and recall $N(\pi)$ represents number of blocks $b$ in $\pi$, $S = \{\pi,  (\phi, \psi) \in A_\pi\}$, $N^* = \underset{\pi\in S}\max$ $N(\pi)$, $S^* = \{\pi,  (\phi, \psi) \in A_\pi \text{ and } N(\pi) = N^*\}$ we have 
\begin{eqnarray*}
    p(A_{\pi}^* | z^1, z^2) \xrightarrow[n\rightarrow\infty]{\text{a.s.}}\left\{
                \begin{array}{ll}
                 m(\pi) \quad  \pi \in S^* \\
              ...
              \right.
\end{eqnarray*}
and $\sum_{\pi\in S^*,} m(\pi) = 1$\\
\end{theorem}\hfill\\
Things become more complicate when parameter satisfy partial intersection constraints, unlike the case for theorem 2, for two different subspaces $A_{\pi_1}$ and $A_{\pi_2}$ covering the parameters, they have refinement relation. But now $A_{\pi_1}$ and $A_{\pi_2}$ covering the parameter do not necessarily have refinement relation. Consequently, for $\pi\in S^*$ , posterior probability of $A_\pi$ will go to some constant $0< m(\pi)< 1$. 
...
3.\textit{Number of subtypes.} Given a number of clusters\\   
...
We determine the number of clusters by searching a range of candidates (from 1 to 9). Given a number of cluster, we compute the posterior probabilities of genes being differential distributed by step 4 - 6. We visualize the change between posterior probabilities under number of clusters $i$ and $i + 1$($i$ from 1 to 8). It typically remains stable when number of cluster is above a number smaller than 9 (figure 4)\\
\begin{figure}[H]
\minipage{0.3\textwidth}
  \includegraphics[width=\linewidth]{sim_56.png}
\endminipage\hfill
\minipage{0.3\textwidth}
  \includegraphics[width=\linewidth]{sim_67.png}
\endminipage\hfill
\minipage{0.3\textwidth}
  \includegraphics[width=\linewidth]{sim_78.png}
  \endminipage
\caption{}
\end{figure}
\hfill\\
...
\end{tabular}\\
\begin{figure}[h]
\includegraphics[scale = 0.5]{heat2.png}
...
\appendix
\section{}
proof of theorem 2 and theorem 3: given the condition that $\alpha_k = 1, \forall k$ and $\beta_b = \sum_{k\in b} \alpha_k$, we can simplify $p(z^1, z^2 | A_\pi^*)$ and obtain $$p(z^1, z^2 | A_\pi^*) = \prod_{b\in \pi}\frac{ \Gamma(\beta_b + t_b^1 + t_b^2)}{\Gamma(\beta_b + t_b^1)\Gamma(\beta_b + t_b^2)}\frac{\Gamma(n + 1)\Gamma(n+1)\Gamma(K)}{\Gamma(2n + K)}$$

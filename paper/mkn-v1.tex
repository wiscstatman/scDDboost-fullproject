...
After finishing calculating $p(z^1,z^2|A^*_\pi)$, we obtain posterior $p(A^*_\pi|z^1,z^2)$ by assign weakly informative prior of $A^*_\pi$ ($p(A_\pi^*)$ is constant not depend on $A_\pi^*$) and obtain $p(A_\pi | z^1, z^2)$ by summing over all the posterior probabilities of $A^*_{\pi'}$, $\pi'$ refines $\pi$.\\

\subsection{asymptotic properties}\hfill\\
\begin{theorem} assuming $\beta_b = \Sigma_{k\in b} \alpha_k$, and $\alpha_k = 1, \forall k$, let $n = min(n_1, n_2)$ be the smaller number of cells of two conditions and $\frac{n_1}{n_2} = O(1)$, when parameter $(\phi, \psi)\in \Omega\setminus Q $ we have 
\begin{eqnarray*}
...
\end{theorem}\hfill\\
Things become more complicate when parameter satisfy partial intersection constraints, unlike the case for theorem 2, for two different subspaces $A_{\pi_1}$ and $A_{\pi_2}$ covering the parameters, they have refinement relation. But now $A_{\pi_1}$ and $A_{\pi_2}$ covering the parameter do not necessarily have refinement relation. Thus, for $\pi\in S^*$ , posterior probability of $A_\pi$ will go to some constant $0< m(\pi)< 1$. 

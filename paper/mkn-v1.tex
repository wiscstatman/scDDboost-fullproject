...
population classes $K$ in order to mitigate that problem. A risk in this is if K approaches n1+n2, wherein every cell is it's own type.
In spite of this theoretical possibility, the data sets often encountered appear not to display this phenomenon,
...
\begin{theorem}  Let $C_{g,\pi} = A_\pi\cap M_{g, \pi}$,  for any different partitions $\pi_1,\pi_2$, $C_{g,\pi_1} \cap C_{g,\pi_2} = \emptyset$. At a given gene, equivalent distribution is
\begin{eqnarray*}
\text{ED}_g = \bigcup_{\pi \in \Pi} C_{g,\pi}.
\end{eqnarray*}
\end{theorem}
 
Our first construction is to quantify probability of a gene being differentially distributed, which is equivalent to quantify $C_{g,\pi}$ based on theorem 1.  We further assume the independence of $A_\pi$ and $M_{g,\pi}$ as proportions $(\phi,\psi)$ are unrelated to the mean expression $\{\mu_{g,i}\}$ of subtypes. By combining cell-type counts $z^1$ and $z^2$ with
...
Prior to any bayesian inference, we need to identify subtypes of cells. Recall subtype is a group of cells with distributions of transcripts that are specific to this group, regardless which condition the cells is from. Thus identification process is blind to conditions and can be done by clustering upon similarities between cells. Denote $X$($G \times (n_1 + n_2)$) as the full data matrix and following is our basic workflow
\begin{algorithm}
\caption{}\label{alg:scDDboost}
\raggedright\hspace*{\algorithmicindent} \textbf{Input}: normalized transcripts profile $X$ and condition label $L$ \\
\hspace*{\algorithmicindent} \textbf{Output}: posterior probabilities of genes being differential distributed
\begin{algorithmic}[1]
\Procedure{scDDboost}{$X$}
\State $z^1, z^2\gets \text{identify subtypes of pooled cells} (X)$ and $L$
\State $P(M_{g,\pi} | X)\gets \text{EBSeq}$\cite{ref:Leng}
\State $P(A_\pi | z^1, z^2)\gets \text{Empirical Bayes}$
\State $P(\text{ED}_g | X, L)\gets \underset{\pi}{\sum}P(M_{g,\pi} | X)P(A_\pi | z^1, z^2)$
\State $P(\text{DD}_g | X, L) \gets 1 - P(\text{ED}_g | X, L)$
\State \textbf{return} $P(\text{DD}_g |X, L)$
\EndProcedure
\end{algorithmic}
\end{algorithm}

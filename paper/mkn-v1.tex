...
\includegraphics[width = \textwidth]{density.pdf}
 \caption{ Densities of log transformed transcripts 25 DD genes uniquely identified by scDDboost, for data GSE75748, DEC vs. EC, We observe some of the genes are different distributed across conditions. *** Need better examples, some genes are quite similar, some genes vanish completely at one condition which is usually not considered in other methods.***}
  \label{fig:6}
\end{figure}
...
By Taylor's expansion $\text{log}(1+x) = \text{log}2 + \frac{1}{2}(x - 1) + O( (x-1)^2)$, we have $\text{log}(1 + \frac{t_b^2}{\beta_b + t_b^1 -1}) = \text{log}2 + \frac{1}{2}(\frac{t_b^1 - t_b^2 + 1 - \beta_b}{\beta_b + t_b^1 -1}) + O_p((\frac{t_b^1 - t_b^2 + 1 - \beta_b}{\beta_b + t_b^1 -1})^2)$ and under condition $\Phi_b = \Psi_b, \frac{t_b^1 - t_b^2 + 1 - \beta_b}{\beta_b + t_b^1 -1}$ is $O_p(\frac{1}{n})$. Plug in $g(n,b)$\\
$$g(n,b) = \text{log}2 * t_b^1 + \text{ln}2 * t_b^2  - (\beta_b - \frac{1}{2})\text{log}(\beta_b + t_b^1 + t_b^2 - 1) + O_p(1) + O_p(\frac{1}{n})$$
and sum up 
\[\sum_{b\in\pi} g(n,b) = 2n\text{log}2 - \sum_{b\in\pi}(\beta_b - \frac{1}{2})\text{log}(\beta_b + t_b^1 + t_b^2 - 1) + O_p(1) +  O_p(\frac{1}{n})\tag{2} \]

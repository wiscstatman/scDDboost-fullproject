...
$$g(n,b) = \text{log}2 * t_b^1 + \text{log}2 * t_b^2  + \frac{1}{2}\text{log}(\beta_b + t_b^1 + t_b^2 - 1) + O_p(1) $$
and sum up 
\[\sum_{b\in\pi} g(n,b) = 2n\text{log}2 + \sum_{b\in\pi}\frac{1}{2}\text{log}(\beta_b + t_b^1 + t_b^2 - 1) + O_p(1)  \tag{2} \]
Notice that when two partition $\pi_1$, $\pi_2$ have same number of blocks $b$ and $\Phi_b = \Psi_b$, $\forall b \in \pi_1\cup\pi_2$, 
\begin{align*}
\sum_{b\in\pi_1} g(n,b) - \sum_{b'\in\pi_2} g(n,b') &=\sum_{b\in\pi_1}(\frac{1}{2})\text{log}(\beta_b + t_b^1 + t_b^2 - 1) - \sum_{b'\in\pi_2}(\frac{1}{2})\text{log}(\beta_b' + t_{b'}^1 + t_{b'}^2 - 1) +  O_p(1)\\

...
Here we leverage well-established modeling tools for transcript analysis.   On $M_{g, \pi}$, cells within
the same subtype, say all $c$ such that $z_c=k$, have $X_{g,c} | z_c = k \sim \text{Negative Binomial}(\sigma_g,\mu_{g,k})$, 
%We use the empirical bayesian scheme from EBSeq \citep{ref:Leng} for the inference. Specifically, for a transcript given the subtype label $X_{g,s} | z_s$, we assume it is negative binomial distributed with hyper parameters($r_g, q_g^{z_s}$).  
%\begin{eqnarray*}
%P(q_g^1,q_g^2,...,q_g^K, M_{g,\pi}) = P(M_{g,\pi})P(q_g^1,q_g^2,...,q_g^K | M_{g,\pi})
%\end{eqnarray*}
and the posterior probability is calculated from
\begin{eqnarray*}
...
&= P(M_{g,\pi})\underset{b\in\pi}{\prod} P(X_{g,c}, z_c \in b | \mu_{g,i} = \mu_{g,j}, \forall i,j \in b)\\
%&= P(M_{g,\pi})\underset{b\in\pi}{\prod} P(X_{g,s}, z_s \in b | q_g^i = q_g^j, \forall i,j \in b).
\end{align*}
One important feature of EBSeq \citep{ref:Leng} is that marginal density of $M_{g,\pi}$ are shared by and estimated from the whole genome.
Mass of DE patterns on one gene depends on mass of DE patterns on other genes, which increase accuracy of our methods Figure~\ref{fig:EBSeq}.
Hyper-parameters are estimated %$(P(M_{g,\pi}),\alpha,\beta^g)$%
through first-order EM algorithm. (Details of calculation can be found at supplementary material 2.2.1)
...
We utilized  this software (version 1.2.0) 
...
we entertained 12 different parameter settings for a two-group comparison on 17421 genes with 10\% DD genes.  Settings
...
of expression levels. These settings were chosen to cover a range
...
of \verb+scDDboost+ compared to three other methodologies: \verb+MAST (1.4.0)+, \verb+scDD (1.2.0)+, and 
\verb+DESeq2 (1.18.1)+. 
...
\begin{figure}[H]
  ...
  \caption{Empirical false discovery rate of methods in scenarios from Figure~4 }
  \label{fig:sim2}
\end{figure}
...
and involving hundreds to thousands of cells.   Supplementary Table 1 provides details.  Figure~\ref{fig:es} compares
methods in terms of the size of the reported list of DD genes at the 5\% FDR target level.  
We see a consistent improvement of gene yeild compared to the other tested methods.  For reference, one of these data 
sets (GSE64016) happens to be the data behind Figure~1, where we know from other information that some
...
To check that the high yeild of \verb+scDDboost+ is not associated with an increased rate of false positives, we 
...
**{\em other methods use BH adjusted pvalue}**


%We validate false positives on nine null datasets from supplementary table 2. For each null dataset, we randomly split the cells from one condition into two subsets and test difference of gene expression between those subsets. Since the two subsets of cells actually came from same condition, there should not be any differential distributed genes, any positive call would be a false positive. We repeat the random split and testing for five times on each null data set. We evaluate the type I error control for the methods returning nominal p-values, by recording the number of genes(with a valid p-value) that are assigned a nomial p-value below 0.05 (Figure 8).\\
%We found bulk methods  {\em DESeq2} has a higher number of false positives comparing to other single cell methods, which also coincides with our observation from the simulation studies.
%{\em scDDboost} could control FDR, essentially we assume cells are sampled from a global population composed of different subtypes. Cells from one subtype are equal likely to be assigned to either one of the two subsets, it is very likely that proportions of subtypes remain unchanged among the two subsets.



\begin{figure}[H]
\includegraphics[width = 0.8\textwidth]{Figs/fdrCounts.pdf}
 \caption{False positive counts by several methods on  null 5 random splits of 9 single condition
data sets from Appendix Table 2} \label{nullperm}
\end{figure}

%%dataset from supplementary table 2, DESeq2 usually identified more false positives compare to other scRNA methods.  I found that one identified more false positives by us is the data EMTAB2805, which contains 45686 genes, 1700 / 45686 = 0.037, so we still control the FDR. More over I tried different 10 shuffles of EMTAB2805, all ends up in very small false positives. So I still use the counts as y axis but using different shuffled null sets of EMTAB2805}
...
leaving gene-level data to handle gene-specific mixture components.   One way to drill into this
idea  is presented in Figure~\ref{fig:clussz}.  In three data sets, we report on genes that are
...
Too big $K$ may end up in an overfit such that two subtypes can be very similar, given we have fixed number of samples (cells), allowing more clusters will introduce more patterns (both for mean expression change and proportion change) to infer and thus reduce our estimation efficiency on those patterns. Overestimating $K$ in \textsc{scDDboost} may losing FDR control (supplementary Figure 8). 

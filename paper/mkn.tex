...
Then, we compare results from \textsc scDDboost to DESeq2, MAST and scDD. 
...
Model performance on the simulated data was assessed based on (1) ability to estimate number of groups and to cluster cells correctly, (2) ability to detect DD genes.
...
\hline
K & $\hat{K_1}$ &  $\hat{K_2}$ & $\hat{K_3}$ & $\hat{K_4}$ \\ \hline \hline
4 & 1 & 2 & 3  & 4 \\ \hline
7 & 1 & 2 & 3  & 4  \\ \hline
15 & 1 & 2 & 3  & 4  \\ \hline
\end{tabular}
\captionof{table}{underlying number of groups $K$ v.s. estimated number of subtypes $\hat K$ under two hyper parameters setting, $\hat{K_1}, \hat{K_2}$ are estimated $K$ of two replicates under the setting $\theta = -0.1$, $\gamma = -1$, $\hat{K_3}, \hat{K_4}$ are for case $\theta = 0.3$, $\gamma = 0.5$ }
\end{table}

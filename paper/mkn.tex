...
of profiles in a way that is blind to the condition label $y$, in order to have as many cells as possible
...
posterior probabilities $P(A_\pi|y, \hat z)$ and $P(M_{g, \pi}| X, \hat z)$ necessary for scoring
...
also conveniently computed for negative binomial counts using \verb+EBSeq+\cite{ref:Leng}. For completeness
we review in Appendix~A the empirical Bayes model underlying \verb+EBSeq+.  Algorithm~\ref{alg:scDDcore}
...
We invoke distance-based $K-$medoids (cite) 
as the default clustering method in \textsc{scDDboost}, and customize the cell-cell distance by integrating two metrics.  
The first metric assembles gene-level information by cluster-based-similarity partitioning (\cite{ref:cspa}).
 Separately at each gene,   modal clustering (\cite{ref:dahl}) partitions the cells, and
...
A second metric defines dissimilarity by 1 - Pearson correlation between cells, which is computationally inexpensive,
less sensitive to outliers than Euclidean distance, and effective at detecting cellular clusters in scRNA-seq(\cite{Cor}).
 We combine the two metrics by a weighted average, 
with  $w_C = \frac{\sigma_C}{\sigma_C + \sigma_P}$ and $w_P = 1 - w_E$. where $w_C,\sigma_C, w_P, \sigma_P$ are the weights and standard deviations of cluster based distance and Pearson correlation distance accordingly.   **XM, please clarify**
...
deviates for hyper-parameters $(\hat a, \hat b)$ derived from $D$.   We argue (Appendix B) that the distribution of
...
In general, we observed that averaged adjusted rand index as well as rand index of mode of partitions based on randomized distance matrices is higher (better estimation) than that of partition based on original distance matrix (supplementary material). 

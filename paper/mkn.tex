...
And the crucial condition for $f$ is that if $y_{S_1}$ and $y_{S_2}$ are overlapped in the sense that  min$\{y_{S_2}\}  <$ max$\{y_{S_1}\}  <$ max$\{y_{S_2}\}$ or min$\{y_{S_1}\}  <$ max$\{y_{S_2}\}  <$ max$\{y_{S_1}\}$, $y_{S_1^*} and y_{S_2^*}$ be the sets of swapping one pair of those overlapped terms and keep the other unchanged. Then $f(y_{S_1}) f(y_{S_2}) \leq f(y_{S_1^*}) f(y_{S_2^*})$. Under such condition, we know that possible MAP candidates must be those partition that for any two subgroups of data, all the data from subgroup1 has to be either greater or smaller than all the data from subgroup2.
...
Under the Poisson-Gamma setting, the model still satisfying 

...
\end{eqnarray*}
We also use the notation $\Phi_\pi = \{ \Phi_b: b \in \pi \}$ and similarly
...
The partition representation is central to our empirical Bayes
posterior probabilities, $P(\text{DD}_g|X,y)$, which we develop below, and
which we use to score differential distribution per gene. We require a 
simplifying prior assumption:  $A_\pi$ and $M_{g,\pi}$ are independent.
...
P(M_{g,\pi}|X).
\end{align}
The methodological  idea is by allowing genome-wide information to inform the posterior of $A_\pi$, then get added benefit from the gene-level data, which primarily drives the posterior of $M_{g,\pi}$. It's a very specific form of information sharing that leverages the compositional structure of single-cell data. 

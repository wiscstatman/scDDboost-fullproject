...
Supposing that the data-generative state $(\phi, \psi) \in A_{\pi_2}$, one might ask how posterior probability mass
tends to be allocated among the other support sets that contain this state.     The question is addressed 
by the following:

\begin{theorem}  Let $(\phi,\psi) \in A_{\pi_1} \cap A_{\pi_2}$ denote the data generating state for group
labels $y_1, y_2, \cdots, y_n$ and subtype labels $z_1, z_2, \cdots, z_n$, and recall the posterior
mixing proportions $\omega^{\rm post}_\pi$ from equation~().  As $n \longrightarrow \infty$
\begin{eqnarray*}
\frac{\omega^{\rm post}_{\pi_1} }{ \omega^{\rm post}_{\pi_2} } \longrightarrow_{a.s.} 0 \qquad 
 {\mbox {\rm if $N(\pi_1) < N(\pi_2)$}}
\end{eqnarray*}
\end{theorem}

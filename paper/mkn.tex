...
Figure~\ref{fig:whet} shows data on several marker genes of G1 or G2/M phase that are called by \verb+scDDboost+ to be
differentially distributed between two populations of cells. These particular genes
were not detected by other standard approaches. Moreover, scDDboost identified 407 cell cycle related genes based annotated in Gene Ontology Cell 
Cycle biological process (GO:0007049) while other methods (MAST, scDD and DESeq2) in total identified 228 genes (see Supplementary Material Section~1 for further details.)  
...
\begin{figure}[H]
\includegraphics[width = 0.7\textwidth]{Figs/density_Fucci_dd.pdf}
 \caption{Cell cycle ralted genes uniquely identified by scDDboost
 from a study of stem cell differentiation (Leng {\em et al.} 2015). Shown are expression
of 4 genes having functionality in cell cycles uniquely identified by scDDboost at 5\% FDR. CALM2 belongs to calmodulin gene family and calmodulin plays a role in cell cycle progression and proliferation.
 CDK4 belongs to cyclin-dependet kinase family that is important for cell cycle G1 phase progression.  TRIM71 is involved in G1-S phase transition of the cell-cycle. And NBN is involved in cell cycle checkpoint control and meiosis.
 \label{fig:whet} }
\end{figure}
...
To better evaluate those methods, we use another two configurations of $(\theta = -0.1, \gamma = 1)$ and $(\theta = 0.3, \gamma = 0.5)$, which zoom in the heterogeneities between subtypes
 and keep the parameters close to default ones.
...
We assess whether \textsc scDDboost can still be powerful when underestimating the number of subtypes.

Considering the above 4 settings of parameters governing the 

We in total generate 24 simulation cases, each case we have 400 cells and we have the roc curve of those 4 methods on the 12 simulation cases below
...
\begin{figure}[H]
  ...
  \caption{True positive rate of four methods under 3 settings of number of subtypes and 4 hyper parameters govern the difference between subtypes, threshold = 0.05, ordered by true positive rate of scDDboost}
  \label{fig:4}
\end{figure}
...
\begin{figure}[H]
  \includegraphics[width = 0.8\textwidth]{Figs/simuFDR.pdf}
  \caption{FDR of four methods under 3 settings of number of subtypes and 4 hyper parameters govern the difference between subtypes, threshold = 0.05,ordered by true positive rate of scDDboost same order as Fig 4}
  \label{fig:5}
\end{figure}
...
\begin{figure}[H]
\includegraphics[width = 0.5\textwidth]{Figs/clussz.pdf}
 \caption{  hexbin plot of cluster size under log scale vs PDD of genes uniquely identified by scDDboost. 
 For each gene, we have an associated cluster label for the DE pattern between subtypes having the maximum of posterior probability. 
 For the whole genome, we count the size of each cluster given by the DE pattern. We expected those uniquely identified DD genes having large proportion on the more frequent DE patterns shared among genes (corresponding with larger cluster size) .}
  \label{fig:7}
\end{figure}

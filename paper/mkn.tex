...
useful in related model-based approaches (e.g., \citep{ref:MAST}; \citep{McDavid:2014aa};
\citep{Huang:2018aa}).
...
statistics in large-scale testing (\citep{Efron:2007aa}; \citep{Muralidharan:2010aa}; \citep{Newton:2004aa}).  
...
Averaging over results from randomized clusterings 
 gives additional stability to the posterior probability statistics.(Supplementary Figure S10).
...
\includegraphics[width = 0.9\textwidth]{Figs/shift.pdf}
 ...
 identified by scDDboost (red) compared to other DD genes (blue), at 5\% FDR, as indicated in three example data sets.
...
\caption{ Absolute values of log fold changes of bursting parameters tend to be larger for genes uniquely identified by scDDboost (red) compare to other genes (blue). at 5\% FDR}
 \label{fig:bursting}
\end{figure}
...
the longest-leg path distance (e.g., \citep{Little2017PathBasedSC}).
...
For computational reasons we impose the rule $K \leq 9$ in
 \verb+scDDboost+ (v. 1.0).  Typically, for a dataset containing 20000 genes and 200 cells.  
 When $K \leq 6$, it takes \texttt{scDDboost} less than 15 minutes for computation using a 4 cores MacBook Pro laptop 
 with 2.2 GHz Intel Core i7 and 16 Gb RAM. 
The computation time increases exponentially with $K$ and it would take 10 to 20 hours under $K = 9$. 
This problem traces to \verb+EBseq+, which  currently 
...
{\em Since we are applying DE methods through many datasets. We select those computationally efficient methods and did not exhaust all DE methods.}

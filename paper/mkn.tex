...
sharing that leverages the compositional structure of single-cell data. 
...
\raggedright\hspace*{\algorithmicindent} \textbf{Input}: \textsc{genes} by \textsc{cells} expression data 
 matrix $X$; \\
\hspace*{\algorithmicindent} \textsc{cells} by \textsc{cells} distance matrix $D$; \\ 
\hspace*{\algorithmicindent} cell condition labels $y$; number of cell subtypes $K$ \\
\hspace*{\algorithmicindent} 
\textbf{Output}:  posterior probabilities of differential distribution from estimated subtypes
\begin{algorithmic}[1]
\Procedure{scDD-core}{$X,y,D,K$}
\State cell clustering: $\hat z = \hat z(D,K) \gets$ labeled parition of cells into $K$ classes
\State subtype differential expression: $\forall g,\pi$,  $P(M_{g,\pi} | X, \hat z)\gets$ using EBSeq\cite{ref:Leng}
...
\raggedright\hspace*{\algorithmicindent} \textbf{Input}: 
 \textsc{genes} by \textsc{cells} expression data matrix $X$; \\
\hspace*{\algorithmicindent} cell condition labels $y$; number of cell subtypes $K$; \\
\hspace*{\algorithmicindent} 
number of randomized clusterings $n_r$; regularization parameter $\lambda$   \\
...
\State $P^* \gets\; $scDD-core$(X,y,D^*,K)$

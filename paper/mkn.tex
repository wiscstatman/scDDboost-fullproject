...
{\em briefly describe EB-seq without a ton of detail, but getting at the product formulation}
...
function, like that induced by the $K-$medoids algorithm.  Then posterior uncertainty in $z$ would follow 
from  posterior uncertainty in $\Delta$.  We could proceed via formal Bayesian analysis, say under
a simple conjugate prior in which $1/\delta_{i,j} \sim $Gamma$(a_0,d_0)$ and in which the observed distance
$d_{i,j} | \delta_{i,j} \sim $Gamma$(a_1, a_1/\delta_{i,j})$, which would assure that $\delta_{i,j}$ is
the expectation of $d_{i,j}$, and with additional parameters $a_0, d_0, $ and $a_1$.  Not accounting
...
$c>0$, we would find
\begin{eqnarray*}
P\left( \delta_{i,j} \leq c | D \right) = P\left( U \geq  \frac{c( d_0+a_1 d_{i,j})}{ a_0+a_1} \right)
\end{eqnarray*}

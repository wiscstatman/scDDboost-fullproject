...
\end{enumerate}
These modeling assumptions imply a useful factorization marginally to latent mean values,
\begin{eqnarray}
\label{eq:ebseq}
...
false discovery rate in all cases while also delivering a relatively high rate of true positives in all cases.
...
\begin{figure}[H]
  ...
  \caption{Empirical false discovery rate of methods in settings from Figure~\ref{fig:sim1} }
  \label{fig:sim2}
\end{figure}
...
are also identifed by our comparison group of methods (\verb+MAST+, \verb+scDD+, or \verb+DESeq2+). 
 The vertical axis reports a model-based cluster size associated
...
the subtypes. Say $\hat \pi_g = {\rm argmax}_{\pi} P( M_{g,\pi} | \hat z, X)$.  Figure~\ref{fig:clussz} shows
that compared to DD genes commonly identified by multiple methods (blue), the cluster sizes of genes
...
matches a large number of other genes. ** {\em Xiuyu, please change colors in this Fig to blue and red; please
 drop the title `right shifts in DE pattern distribution`, and please use the GSE id in the horizontal label;
you know as I look more at the boxplots I suspect that ecdf's might look better; perhaps you can draft one
to see} **
...
\begin{figure}[H]
\includegraphics[width = 0.8\textwidth]{Figs/shift.pdf}
 \caption{Clusters of genes tend to be larger (horizontal axis, log size) for genes uniquely
 identified by scDDboost (red) compared to other DD genes (blue), as indicated in three example data sets.
 Here genes are clustered by their pattern of differential expression across subtypes as inferred by
the EBseq computation within scDDboost.}
  \label{fig:shift}
...
**{\em Xiuyu, this section needs to be more carefully written. Please take a crack; explain the essense of D3E as
an attempt to get at a mechanistic model; right now far too much is in the Figure caption}...**

Transcriptional bursting is a fundamental property of genes. In its simplest form, the gene is proposed to exist in two states, one where activity is negligible and one where there is a certain probability of activation\citep{Raj:2008aa}. Composition of multiple states yields subtle change of transcripts between conditions. 
D3E\citep{ref:d3e} is a method for DE gene analysis rooted in quantifing the parameters of bursting process. It modeled transcripts as the stationary distribution from a widely used, experimentally validated stochastic process of single-cell gene expression \citep{Peccoud:1995aa}. 
Three biologically meaningful parameters (rate of promoter activation, rate of promoter inactivation and the rate of transcription when the promoter is in the active state) characterize the model, which made
D3E capable of detecting distributional change when the mean level remains unchanged.
We investigate DD genes identified by scDDboost and their changes of those three parameters on dataset GSE71585 (Figure~\ref{fig:bursting}).

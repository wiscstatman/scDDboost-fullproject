...
\begin{eqnarray}
\label{eq:ddmix}
p(\phi,\psi) = \sum_{\pi \in \Pi} P(A_\pi) \, p_\pi(\phi,\psi ).
\end{eqnarray} 
The multitude of $P(A_\pi)$'s are taken to be preset constants.  ({\em MAN to XM: All equal?})
Each mixing component $p_\pi(\phi,\psi)$ has support $A_\pi$.
To complete the specification, introduce scalers
$\alpha_k>0$ for each subtype $k$, and $\beta_b>0$ for each possible block $b$.
...
\end{eqnarray*}
which indicate the conditional probability of each subtype $k$ given that
the cell is of one of the subtypes in $b$.  Assume that conditional upon 
$A_\pi$,
\begin{eqnarray*}
...
\end{eqnarray*}
where $N(b)$ is the number of cell subtypes in block $b$.
In other words, if $A_\pi$ is the active structure, then
accumulated probability vectors $\Phi_\pi$ and $\Psi_\pi$ are equal
between the two source conditions, though the sub-block subtype-specific
...
these distributions (\cite{ref:Dickey}), we deduce some properties of the Double-Dirichlet model.

\noindent
{\bf Property 1:}  In $p_\pi(\phi,\psi)$, $\psi$ and $\phi$ are dependent but 
  marginally identically Dirichlet distributed.

\noindent
{\bf Property 2:} The predictive distributions of cell-type counts $t^1$ and $t^2$ (\ref{eq:counts})
 are all available in closed form..

Fixing a partition $\pi$,
let $t^j_b = \sum_{k\in b} t^j_k$, for cell conditions $j=1,2$,
record the total numbers of cells accumulated over all types in block $b \in \pi$.
Following our notation convention, $t^j_\pi$ is the vector of these
counts over $b \in \pi$.  Further, we use $\phi_b$ to denote the vector of values $\phi_k$ for $k \in b$,
...
From the prior and model structure
\begin{eqnarray*}
...
We have presented scDDboost, a compositional model for detecting differential distributed genes from scRNA-seq data. To account for the over-dispersion and multi-modality of single-cell data, scDDboost modeled transcripts as mixture distributed. Unlike previous invented methods (e.g. Deseq2, MAST and scDD), which conducts genewise DD test in an isolated manner. scDDboost make whole genome information shared at gene level by further assuming the mixture distribution of transcripts is a mixture over the subtypes of cells. We combine estimations of changes of subtypes' proportions across conditions and changes of mean expressions across subtypes to infer distributional changes of transcripts.
To estimate changes of subtypes' proportions across conditions, we use empirical Bayes and developed a double dirichlet prior distribution.  We invented a random weighting scheme that stabilize our DD inference as well as having the property to approximate the results from a fully baeysian framework. scDDboost outperforms existing approaches on the simulation study and tends to be more powerful than existing methods on empirical studies. 

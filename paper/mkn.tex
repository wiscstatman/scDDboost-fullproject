...
where 
\begin{align*}
P(M_{g,\pi}, X | z) &= P(M_{g,\pi})P(X | M_{g,\pi}, z)\\
...
On $M_{g, \pi}$, cells within
the same subtype, say all $c$ such that $z_c=k$, have $X_{g,c} | z_c = k \sim \text{Negative Binomial}(\sigma_g,\mu_{g,k})$, 

One important feature of information sharing is that marginal density of $M_{g,\pi}$ are shared by and estimated from the whole genome.
Mass of DE patterns on one gene depends on mass of DE patterns on other genes.

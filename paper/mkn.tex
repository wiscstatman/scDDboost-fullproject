...
of \verb+scDDboost+ compared to three other methodologies: \verb+MAST(1.4.0)+, \verb+scDD(1.2.0)+, and \verb+DESeq2(1.18.1)+. 
...
12 data sets from \verb+conquer+  ({\em how?} randomly selected) representing different species and experimental settings
and involving hundreds to thousands of cells.   Supplementary Table 1 provides details.  Fig.~6 compares
...
%method, (\verb+sc3+~\citep{sc3}, Supplementary Fig ***)  and obtain similar results as the default one.
...
Too big $K$ may end up in an overfit such that two subtypes can be very similar, given we have fixed number of samples (cells), allowing more clusters will introduce more patterns (both for mean expression change and proportion change) to infer and thus reduce our estimation efficiency on those patterns. Overestimating $K$ in \textsc{scDDboost} may losing FDR control (supplementary Fig 8). 

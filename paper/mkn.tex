...
These sets, which are all closed, convex subsets of $\Theta$, are precisely the
...
Here we describe the partition-reliant prior $p(\phi,\psi)$ indicated in~Figure~2 and an explicit
formula for $P(A_\pi|y,z)$.  The prior is conjugate to multinomial
...
the mixing proportions $\omega_\pi$ are any non-negative constants summing to one. In numerical
experiments we have taken $\omega_\pi \propto 1$.  **{\em MAN to XM: what values do we use for alphas and betas?}**
To specify component $p_\pi$,  introduce scalers
$\alpha_k>0$ for each subtype $k$, and $\beta_b>0$ for any possible block $b$.
Extending notation, let $\alpha_b$ be the vector of $\alpha_k$
...
of $\Phi_b$ and $\Psi_b$ for $b \in \pi$.

Notice that on $A_\pi$ there is a 1-1 correspondence between pairs $(\phi, \psi)$ and 
\begin{eqnarray}
\label{eq:onetoone}
...
is one of the subtypes in $b$. The proposed double-Dirichlet component 
$p_\pi$ is determined in this transformed scale by assuming:
\begin{eqnarray}
\label{eq:doubledir}
\Phi_\pi  \sim \text{Dirichet}_{N(\pi)}[   \beta_\pi   ] , \; {\mbox {\rm and independently}}, \;
\tilde \phi_b, \tilde \psi_b \sim_{\text{i.i.d.}}
  \text{Dirichlet}_{N(b)}[ \alpha_b ]
\end{eqnarray}
where $N(\pi)$ is the number of blocks in $\pi$ and $N(b)$ is the number of subtypes in $b$.
In other words, on $p_\pi$,
accumulated probability vectors $\Phi_\pi$ and $\Psi_\pi$ are equal
between the two source conditions, though the  subtype-specific
rates $\phi_k$ and $\psi_k$ may differ, as would (re-normalized)
independent Dirichlet-distributed vectors. (See also Appendix D.) 
 We deduce some properties of the double-Dirichlet model~(\ref{eq:doubledir}), 
using the Dirichlet-Multinomial conjugacy and the collapsing property (\cite{ref:Dickey}). 

\noindent
{\bf Property 1:}  In $p_\pi(\phi,\psi)$, $\psi$ and $\phi$ 
  marginally identically distributed as Dirichlet$()$.

\noindent
{\bf Property 2:}  In $p_\pi(\phi,\psi)$, $\psi$ and $\phi$ are dependent, unless $\pi$ is the complete
partition in which every cell subtype is its own block.
...
\noindent
{\bf Property 3:}  $t^1$ and $t^2$ are conditionally independent given $y$.
...
Case 2. At the opposite extreme, $\pi$  has one block $b$ for each
 class $k$. Then $p_\pi(t^j | t^j_{\pi},y) = 1$, and 
...
\noindent
{\bf Property 6:} Posterior:
\begin{eqnarray*}
p(\phi,\psi|y,z) = \sum_{\pi \in \Pi} \left[ \omega_\pi^* \right]  p_\pi( \phi, \psi | t^1, t^2, y )
\end{eqnarray*}
...
on the non-disjointness, on refinements, etc

...
One advantage of our approach is that the posterior inference can be incorporate with different clustering methods. The PostI function 
...
\hspace*{\algorithmicindent} \textbf{Output}: labelled partition of cells $\hat z$
\begin{algorithmic}[1]
...
\State $\hat z\gets K\text{-medoids on }\hat{D}$
\State \textbf{return} $\hat z$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\State $P(\text{DD}_g | X, y) \gets$ PostI($X, y, \hat z$)
\State $i\gets i + 1$
\EndFor

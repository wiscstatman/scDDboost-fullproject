...
The EBSeq model assumed a prior distribution on $q_g^I : q_g^I | \alpha, \beta_{I_g} \sim Beta(\alpha, \beta_{I_g}).$ The hyperparameter $\alpha$ is shared by all the isoforms and $\beta_{I_g}$ is $I_g$ specific. We further assume that $r_{g,s} = r_{g,0}*l_s$ where $r_{g,0}$ is an isoform specific parameter common across subtypes and $r_{g,s}$ depends on it through the sample-specific normalization factor $l_s$ 

What we are interested at those $K$ groups comparison is the expression pattern, the hypotheses are:

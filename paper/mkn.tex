...
\begin{figure}[h!]
  ...
  \caption{first two principal components of transcripts under different parameters for simulated data. Different parameters resulted in different degree of separation of subtypes.}
  \label{fig:4}
\end{figure}
...
In this section, we gave an intuitive justification for consistency between bayesian framework clustering analysis and random weighting procedure. A fully bayesian analysis for clustering needs two specifications of likelihood. First one is the prior for partition and second one is the density of data given the partition. Specifically, in single cell case we need to know the density of transcripts of genes given the partitions which requires understanding of co-expression and dependence between genes. It is infeasible to do a strict bayesian analysis on the clusters. Nevertheless we    

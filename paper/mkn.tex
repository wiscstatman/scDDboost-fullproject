...
of three genes identified by scDDboost at 5\% FDR, but not similarly identified by other approaches. 
 ...
 Heatmap (right) shows these three genes among **how many**  other cell-cycle genes (GO:**). 
 \label{fig:whet} }
\end{figure}
...
$w_{i,j} = ( e_i + e_j )$, and where $( e_i) $ are independent and identically Gamma$(\frac{\hat a}{2}, \hat a)$ distributed
deviates for hyper-parameters $ \hat a$ derived from $D$.   
The distribution of
...
We use the empirical bayesian scheme from EBSeq \citep{ref:Leng} for the inference. Specifically, for a transcript given the condition label $X_{g,s} | z_s$, we assume it is negative binomial distributed with hyper parameters($r_g, q_g^{z_s}$). Then to score the differential patterns between mean expression $\mu^1,...,\mu_K$ can be equivalently scored for differential patterns between $q_g^1,...,q_g^K$. For such purpose, we assume $q_g \sim \text{Beta}(\alpha, \beta^g)$. 
...
&= P(M_{g,\pi})\underset{b\in\pi}{\prod} P(X_{g,s}, z_s \in b | \mu_g^i = \mu_g^j, \forall i,j \in b)\\
...
\begin{theorem} Let $n = min(n_1, n_2)$ be the smaller number of cells of two conditions and $n_1 = O(n_2)$, and hyper parameters of DDM $\alpha^1, \alpha^2$ be vectors of constants, $\alpha_k^j \geq 1$, $\forall k, j$ and $\beta = \alpha^1 + \alpha^2$. Then if parameter $(\phi, \psi)\in \Omega\setminus Q $ we have 
\begin{eqnarray*}
...
Still using above example, in limiting case, we have $p(A_{\pi_3} | y,z) = 1$, $p(A_{\pi_2} | y,z) = 1$ and $p(A_{\pi_1}| y,z) = 0$. When the DE pattern is $A_{\pi_1}$ for some genes and our estimation of $p(A_{\pi_1}| y,z) = 0$, we will falsely classify those genes as differential distributed.
...
a $n \times n$  matrix holding {\em true} distances, such as  $\delta_{i,j}$ between cells $i$ and $j$, 
and that $f$ is some assignment
function, like the one 
induced by the $K-$medoids algorithm.  Then posterior uncertainty in $z$ would follow 
directly from  posterior uncertainty in $\Delta$.  We could proceed via formal Bayesian analysis, say under
a simple conjugate prior in which $1/\delta_{i,j} \sim \, $Gamma$(a_0,d_0)$ 
and in which the observed distance
$d_{i,j} | \delta_{i,j} \sim \, $Gamma$(a_1, a_1/\delta_{i,j})$, which would assure that $\delta_{i,j}$ is
the expectation of $d_{i,j}$, and using additional parameters $a_0, d_0, $ and $a_1$.  Not accounting
for any constraints imposed by both $D$ and $\Delta$ being distance matrices, we would have the marginal

...
\begin{theorem}  
For each $\pi$, $P\left(A_\pi \cap M_{g,\pi} |X,y,z \right) 
...
Here we leverage well-established modeling tools for transcript analysis. 
EBSeq \citep{ref:Leng} factorize the $P( X_g | M_{g,\pi}, z)$ as product of predictive distribution under the equality constrains within each block $b\in\pi$.
\begin{eqnarray*}
P(X_g | M_{g,\pi}, z) = \underset{b\in\pi}{\prod}P(X_g^k, k\in b | \mu_{g,i} = \mu_{g,j}, \forall i,j \in b)
\end{eqnarray*}
where $X_g^k$ represent read counts at gene $g$ of cells from subtype $k$ and $X_g = (X_g^1,...,X_g^K)$.

%{\em Xiuyu: this presentation still masks some of the essential structure; e.g. the
%factorization aspects are far more critical than the particular Neg. Binom. assumption, and they should
%be presented with the more important stuff first.} 
...
where 
\begin{align*}
P(M_{g,\pi}, X | z) &= P(M_{g,\pi})P(X_g | M_{g,\pi}, z)\\
...
\end{align*}
Notice that left hand side of above formula is $P(M_{g,\pi}, X | z)$ rather than $P(M_{g,\pi}, X_g | z)$.
%cells within the same subtype, say all $c$ such that $z_c=k$, have $X_{g,c} | z_c = k \sim \text{Negative Binomial}(\sigma_g,\mu_{g,k})$, 
The reason is that marginal density of $M_{g,\pi}$ are shared by and estimated from the whole genome, 
which is an important feature for borrowing information from other genes.

...
\includegraphics[width = 0.9\textwidth]{Figs/shift.pdf}
 ...
 identified by scDDboost (red) compared to other DD genes (blue), at 1\% FDR, as indicated in three example data sets.
...
\caption{ Absolute values of log fold changes of bursting parameters tend to be larger for genes uniquely identified by scDDboost (red) compare to other genes (blue). **XIUYU, is this 5 or 1\% FDR?**}
 \label{fig:bursting}
\end{figure}
...
distances based on diffusions (e.g., Haghverdi {\em et al.} 2015 **bioinformatics**) or 
the longest-leg path distance (e.g., Little {\em et al.} 2017 **arXiv:1712.06206**).
...
For computational reasons we impose the rule $K \leq 9$ in
 \verb+scDDboost+ (v. 1.0).  **{\em Xiuyu...we should give a ballpark figure on compute time for end-end
analysis on one data set}**  This problem traces to \verb+EBseq+, which  currently 
searches all partitions of $K$ and encodes a hyper-paramter estimation algorithm that scales poorly with $K$.
...
patterns over subtypes will have a big impact on the final posterior inference.   Even resolving this

...
Time complexity of \texttt{scDDboost} can attribute to two parts: clustering and \texttt{EBSeq}. Recall $n$ for number of cells, $G$ for number of genes and $K$ for number of subtypes. 
The time complexity for clustering on one gene is O($n^2$) and thus in total O($Gn^2$). 
\texttt{EBSeq} use sum of counts within each subtype (sufficient statistics) to compute density kernel and for each gene there are Bell$(K)$ DE patterns to compute, which yields the time complexity: O($n$) + O($G\text{Bell}(K)$). 

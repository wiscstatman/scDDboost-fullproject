...
\author{\fnms{Keegan} \snm{Korthauer}\thanksref{k1}\ead[label=e4]{keegan@stat.ubc.ca}},
...
Department of Statistics, University of British Columbia \thanksmark{k1}}
...
information \textcolor{black}{and therefore may be underpowered in some settings.  The proposed method,
...
prioritizes genes \textcolor{black}{with a local false-discovery rate against the 
...
%according to evidence that their expression distribution is different
%between the two cellular conditions in view.  \textcolor{black}{The goal is to score genes
%by an empirical Bayesian posterior probability that the expression sampling distribution differs
%between these cellular conditions.}
...
\textcolor{black}{(GSE64016)}. 
  Density estimates on the left show expression data (log2 scale)
of three genes identified by scDDboost at 5\% FDR, but not similarly identified by MAST, scDD, \textcolor{black}{or}  DESeq2.
...
in Section~5.  We relegate some details to an appendix and many others to supplementary 
material.
...
when using a broad class of within-subtype expression distributions. \textcolor{black}{Subtypes are
considered such 
...
Details confirming such equality are examplified further in 
Supplementary Material Section 2.1, \cite{Ma20}. The fact is so central that we emphasize:
...
The first assembles gene-level information by cluster-based-similarity partitioning~(\cite{ref:cspa}).
 Separately at each gene,   modal clustering (\cite{ref:dahl} and Supplementary Material Section~2.2, \cite{Ma20}) partitions the cells, and
...
other distances, \textcolor{black}{such as provided by SC3, which we use in some numerical experiments}; in any case 
...
Supplementary Material Section 2.2, \cite{Ma20}, shows good operating characteristics in simulation.
...
uncertainty in the block-specific means (see Supplementary Material Section 2.2, \cite{Ma20}).  
...
variation.  Further details are in Supplementary Material Section~3.1,\cite{Ma20}.
...
\textcolor{black}{The beneficial sampling  properties are not limited to the 5\% FDR threshold, as indicated by
receiver operator characteristic curves (Supplementary Figure~S9).}
...
\begin{figure}[H]
  ...
  \caption{False discovery rate (vertical) of methods in the synthetic-data settings (horizontal, same order) from Figure~\ref{fig:sim1}. FDR is approximated from the 12 replicate data sets in each scenario using the known generative hypothesis states. }
  \label{fig:sim2}
\end{figure}
...
\includegraphics[width = 0.95\textwidth]{Figs/DD95.pdf}
 \caption{Proportion of DD genes at 5\% FDR threshold with respect to total number of genes identified by each method. 
 Data sets are ordered by scDDboost list size. }
  \label{fig:es}
\end{figure}
...
significant changes between estimated bursting parameters \textcolor{black}{than genes that all methods identify}.
...
\caption{ Absolute values of log fold changes of bursting parameters tend to be larger for 1758 genes uniquely identified by scDDboost (red) compared to 2983 genes (blue) \textcolor{black}{that are  identified  at 5\% FDR by 
...
Our distance-based clustering of  $n$ cells measuring $G$ genes requires on the order of $G \times n^2$ operations (see Supplementary Material Section 2.2, \cite{Ma20}).
...
\subsection{\textcolor{black}{Diagnostics}}

\textcolor{black}{As implemented, scDDboost uses a particular distance matrix to inform subtypes and 
...
\textcolor{black}{To check the within-subtype negative binomial (NB) assumption, we deployed a bootstrap 
goodness-of-fit test in three data sets~(\cite{Yin:2013aa}). Fewer than 1.5\% of genes show evidence against
a within subtype NB assumption at a 5\% FDR.
 Further, for most of these non-NB genes the inference drawn by \texttt{scDDboost} is the same as that
drawn by various other methods (Supplementary Table S3), and where there are differences the 
\texttt{scDDboost} call is plausible (Supplementary Figure S12).   Among 
the  genes identified by \texttt{scDDboost} at 5\% FDR in the stem-cell example
(recall Figure~1), just six of them fail the NB test, and two of these are uniquely called by \texttt{scDDboost}; 
further, one of the two genes is cell-cycle related.  }


\textcolor{black}{The constant-shape assumption is less well supported empirically, according to 
a likelihood-ratio test that we developed (Supplementary Figure~S13).  More than 15\% of genes show 
evidence against constant shape in the examples considered, though inference on differential
distribution is only mildly affected  (Supplementary Table~S4; Figure~S14). We have similar findings 
in the splatter-generated synthetic data.  Even with evident model mis-specification, 
Figure~\ref{fig:cdf} shows that
  the marginal fit by \texttt{scDDboost}
is reasonably accurate in the stem-cell example.  }

\begin{figure}[h]
\includegraphics[width = 0.85\textwidth]{Figs/ecdf.pdf}
\caption{
\textcolor{black}{Empirical CDF for observed data (green) compared to expression levels simulated from the fitted NB mixture (pink) for
data set GSE64016. The top 6 panels are genes randomly selected from those genes being identified as DD by \texttt{scDDboost} and not violating constant shape assumption; 
the last three genes are randomly sampled from genes that fail the test of constant NB shape parameter. 
Each thin pink curve is from one of the randomized distances; the
 the thicker pink curve represents pointwise averaging over 10 randomized distance matrices. Note horizontal scales
differ among the panels, and  cells are pooled from the two conditions.}}
\label{fig:cdf}
\end{figure}


\subsubsection*{\textcolor{black}{Choice of distance function}}

\textcolor{black}{We propose distance-based clustering to convey subtype information (Section 2.2), and one may ask
how sensitive are findings to the choice of distance function.  The randomization scheme mitigates variation
associated with the specific partition of cells, but the choice of distance function does affect the computed
probabilities.  To check this,  
we repeated the numerical experiments in Section~3 using the SC3 distance in place of the default. 
Importantly, Type~I error rates remain in control, as evidenced by the simulation study and the negative-control
permutatation study (Supplementary Figure S15).  There can be substantial differences in yeild at
a given FDR control level (Supplementary Table S5).}
...
\end{eqnarray}
This always happens when $K \leq 3$.  In Supplementary Material Section~4, \cite{Ma20}, we characterize 
...
patterns over subtypes have a big impact on the final posterior inference~\cite{Ma2020}.   Even resolving this
...
and US  National Science Foundation grant 1740707. We thank the associate editor and referees for very helpful comments.
...
\sdescription{Additional mathematical and computational results}
\end{supplement}

\begin{supplement}{B}
\stitle{R package}
\sdescription{scDDboost version 2.0}
\end{supplement}

\begin{supplement}{C}
\stitle{Code and data}
\sdescription{Source code required for figures and tables}
\end{supplement}
...
in the supplement make comparisons to a full, Dirichlet-process-based, posterior analysis.

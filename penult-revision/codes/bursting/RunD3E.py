...
def RunD3E(input_dir, output_dir, label1, label2):
    args = ["python", "./D3E-master/D3ECmd.py", input_dir, output_dir, label1, label2, "-m","1","-t",
        "0","-z","0","-n","0","-v"]
    subprocess.run(args)
...
def main(argv):
    ...
    ARGS = os.listdir("./D3E_out/")
    
    label1 = "Gad2"
    label2 = "Cux2"
    proc_args = []
    for file in ARGS:
        input_dir = "./D3E_out/" + file
        INDEX = file.split("_")[2].split(".")[0]
        output_dir = "./D3E_res/res_" + str(INDEX) + ".txt"

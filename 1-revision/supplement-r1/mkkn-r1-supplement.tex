...
\begin{figure}[h!]
  \includegraphics[ width = 0.8\textwidth]{Figs/UMAP.png}
...
\textcolor{red}{We also use t-test to demonstrate the difficulty for different simulation settings.} 
...
\begin{figure}[h!]
  ...
  \caption{for each settings, we randomly choose one data and plot the boxplot of p values by t-test at DD and ED genes. 
  }
  \label{fig:t-s}
\end{figure}
...
%\label{fig:s12}
%\end{figure}
Apart from NB fitting test,
we also investigate how well our model approximates the empirical values.
\begin{figure}[h]
\includegraphics[width = 0.8\textwidth]{Figs/ecdf.pdf}
\caption{Data set Fucci, random select 9 genes among genes having at least one positive transcripts, Red curve refers to empirical CDF of the gene transcripts, each green curve refers empirical CDF fitted on data simulated from our estimated mixture NB model. Here we present 10 fitted models for each gene under randomized clusterings.}
\label{fig:cdf}
\end{figure}

...
\textcolor{red}{Here we look more closely at the synthetic data generated using splatter.
Splatter is a state-of-the-art simulation framework to generate scRNA-seq data.
...
default settings in our experiments.  For generating data across multiple groups, 
splattert requires user to provide a pair $(\theta,\gamma)$ of location and scale 
parameters governing the intensity of DE for those signature genes that distinct each group.
(These parameters are also denoted \bf{de.facLoc} and \bf{de.facScale}.)  We follow  the discussions at:}
...
\textcolor{red}{ We take four settings for $(\theta, \gamma)$: two are defaults used in splatter  and a variation of each
setting to inflate the differential signal: **list the four**}

 so we pick those 2 default pairs of theta and gamma since splatter argued they gave authentic simulated data under some criterion.

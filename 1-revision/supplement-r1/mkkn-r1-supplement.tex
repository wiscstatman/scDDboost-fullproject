...
\verb+splatter+ is a state of art simulation framework that approximates empirical data well.
To make authentic data, gene expression of cells within the same group are generated through a multi-layer model, 
Those layers considering the state of whether it is an outlier or dropout, library size(sequencing depth) and trended gene-wise dispersion, 
which make the marginal expression distribution more complicate than the negative binomial model assumed by \verb+scDDboost+. 
Input parameters for the multi-layer model are internally estimated using empirical data sets. 
However, for generating data of multiple groups, 
it requires user to provide a pair of location and scale parameters governing the intensity of DE for those signature genes that distinct each group. 
In summary, \verb+splatter+ take three parameters, the number of groups(clusters) and a pair of the location and scale parameters for multi-group simulation.
...
\begin{figure}[h!]
  ...
  From left to right, the associated hyper-parameters are (0.1,0.4), (-0.1,0.3), (0.3,0.5), (-0.1,1). Here we have $K=3$ subtypes, with cells in each subtype marked with a common color. 
  **\textcolor{blue}{what are top and bottom rows?**}}
  \label{fig:pca1}
\end{figure}
...
equivalence probabilities to empirical Wasserstein distance \textcolor{blue}{**Xiuyu: please provide
a citation to this metric**}

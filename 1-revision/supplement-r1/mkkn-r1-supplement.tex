...
\vspace{.2in}
\noindent
Version:  \today.  Additions over the original version (May 30, 2019) are indicated in \textcolor{red}{red}.
...
\item Modeling
 \begin{itemize}
  \item 2.1 Data Structure, Sampling Model, and Parameters
    \begin{list}{}{}
    \item \textcolor{red}{Illustrating {\em key issue}}
...
\item Numerical Experiments
   ...
   \item \textcolor{red}{3.5 Diagnostics}
    \begin{itemize}
    \item \textcolor{red}{Negative binomial assumption}
    \item \textcolor{red}{Constant shape assumption}
...
\begin{eqnarray*}
f(X_g^{I,J}) &=& \int_0 ^1 P(X_g^{I,J} | r_{g}, q_g) \k P(q_g | \alpha, \beta^{g}) \, dq_g \\
...
\includegraphics[scale = 0.7]{Figs/ARI.pdf}
 ...
 in 8 data sets and for 100 randomized matrices per data set. 
}
  \label{fig:ARI}
\end{figure}
...
We also check validity of randomized distances  by comparing it to Dirichlet-process-based 
...
\includegraphics[width = 0.5\textwidth]{Figs/breakFDR.pdf}
 \caption{FDR control may be lost if $K$ is too large.  Calculations here are
 **Under NULL case**, using dataset EMTAB2805 and targeting 5\% FDR, which is not controlled when $K>6$
But by the validity score method we would choose $K$ much smaller, as that score stabilizes $K\geq3$. 
 **\textcolor{red}{Xiuyu; what exactly do you mean by null case here?}**}
  \label{fig:lfdr}
\end{figure}
...
\textcolor{red}{
For the choice of the hyperparamter pair, we use two settings suggested by \verb+splatter+, which are (0.1,0.4) and (-0.1,0.3). 
...
\subsubsection{\textcolor{red}{Negative binomial assumption}}

\textcolor{red}{We deploy a Pearson-type goodness-of-fit test~(\cite{Yin:2013aa}) to check the adequacy of the
...
\begin{figure}[h!]
  ...
  \label{fig:nb}
\end{figure}
\textcolor{red}{ Among genes that demonstrate non-NB behavior, we find that most are handled the same way by \texttt{scDDboost}
...
\subsubsection{\textcolor{red}{Constant shape assumption: Likelihood ratio test}}

\textcolor{red}{As implemented, \texttt{scDDboost} treats the negative binomial mixture components as having constant shape
...
\subsubsection{\textcolor{red}{Clustering}}

\textcolor{red}{How sensitive are \texttt{scDDboost} results to the particular clustering methodology?  To investigate, we

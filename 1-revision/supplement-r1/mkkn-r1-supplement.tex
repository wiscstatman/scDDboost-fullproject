...
\begin{figure}[h!]
  ...
  \caption{ROC curve in 12 simulation settings. In each setting, TPR and FPR are averaged over ten replicates, generally \texttt{scDDboost} performs better than other methods, orange dash line refers to y = x all panels show the unit square.}
  \label{fig:roc}
\end{figure}
...
In this section, we provide details of the empirical datasets and also demonstrate an empirical relationship
...
\hline
\end{tabular}
\caption{Datasets used for null cases. Cells arise from one  biological condition in each case.  We 
...
Table \ref{tab:nb} demonstrates among the non-NB genes, most DD identified by our method are also confirmed by other tests, suggesting that model misspecification may not often lead to erroneous inferences.
...
is  repeated 100 times. Though the LRT with chi-square p-value is slightly liberal (Figure \ref{fig:lrt}),
\begin{figure}[h!]
...
Looking more closely we focus on non-constant-shape genes in the stem-cell example. 
Though some of these are  uniquely called as DD by \texttt{scDDboost}, 
...
It can also be validated from simulation, we run the likelihood ratio test for constant shape on the simulated data sets we used previously, and there are roughly 1000 genes violate the constant shape assumption. 
Among those 1000 genes, \texttt{scDDboost} on averaage identified 50 DD genes and on average 49 genes are true DD genes. Thus our method control FDR even when the constant shape condition does not hold.

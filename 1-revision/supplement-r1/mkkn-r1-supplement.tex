...
Here we look more closely at the synthetic data from \verb+splatter+,
 a state-of-the-art simulation framework to generate scRNA-seq data.
...
We notice that the evidence of a true change  across conditions is very weak on these settings,
 that all methods have low power. So we amplify the differential genes with two additional
settings:  $(-0.1,0.3)$[default] -> $(-0.1,1)$ 
and $(0.1,0.4)$[default] ->$(0.3,0.5)$.  These choices enable better resolution for methods comparison.


In summary, \verb+splatter+ take three parameters, the number of clusters $K$ and a

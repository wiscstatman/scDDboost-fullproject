...
For those genes against NB assumption and uniquely identified by \texttt{scDDboost}. From Fig \ref{fig:s11} we found they are somewhat different.
Further we test difference between proportions of zeros on those genes by using t-test and it is favorable for not identical proportions with p values 0.02
\begin{figure}[h]
\includegraphics[width = 0.7\textwidth]{Figs/NBgene.pdf}
\caption{violin plot of the 57 genes having evidence against negative binomial distribution assumption and uniquely called positive under \texttt{scDDboost}}
\label{fig:s11}
\end{figure}

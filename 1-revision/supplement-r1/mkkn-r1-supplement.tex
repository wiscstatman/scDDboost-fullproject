...
\item Modeling
 \begin{itemize}
  \item Data Structure, Sampling Model, and Parameters
    \begin{list}{}{}
    \item Demonstrating {\em key issue}
  ...
  \item Method Structure and Clustering 
    ...
    \item Negative binomial fitting
    \end{list}
...
\noindent
\textcolor{red}{{\bf Illustrating key issue:}}

\textcolor{red}{Consider an example as in Figure 2 [main] wherein there are $K=7$ subtypes having 
proportions $\phi=(\phi_1, \cdots, \phi_7)$ in one condition and proportions 
...
that there are three unique component distributions among the seven, say $\alpha(x), \beta(x)$,
and $\gamma(x)$, and suppose there ties:
\begin{eqnarray*}
\alpha &=& f_{g,3} = f_{g,4} \\
\beta  &=& f_{g,2}= f_{g,5} = f_{g,6} \\
\gamma &=& f_{g,1} = f_{g,7}. 
\end{eqnarray*}
These represent the lack of differential expression between subtypes within each block, but
differences between blocks.  Let's work out the marginal distribution of data in the first
...
f^1_g(x) &=& \sum_{k=1}^7 \phi_k f_{g,k} (x) \\
   ...
   &=& f^2_g(x) .
\end{eqnarray*}
To reiterate the key issue, a gene that does not distinguish several subtypes (ties in
...
\subsection{Negative binomial fitting}
...
\subsection{Inference base on \texttt{SC3}}

Five data sets (EMTAB2805, GSE64016, GSE93818, GSE74596, GSE71585) are used to check the negative control for our method appling sc3 for clustering. 
We use same procedure to generate null case. Namely,
cells from one condition are randomly split to two groups under each data set, thus for any gene, there is no distributional change between the two groups. 
For each data set, five randomly splits are used. We observed zero positives for our method, thus \texttt{scDDboost} can still control the FDR given different clustering method.

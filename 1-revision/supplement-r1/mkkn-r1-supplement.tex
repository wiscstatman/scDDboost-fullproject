...
and observed that a few genes having evidence against the NB assumption 
...
Supplementary Table \ref{tab:nb} demonstrates among the non-NB genes, most DD identified by our method are also confirmed by other tests, suggesting that model misspecification may not often lead to erroneous inferences.
...
Group differences are evident (Supplementary Figures ~\ref{fig:s11}) even from genes that show non-NB behavior
and that are uniquely identified by \texttt{scDDboost} as being DD.   
\begin{figure}[h]
...
\sigma^2 = \dots = \sigma^K,$$ the test statistic is $2(l_1 - l_0)$, where $l_1$ is the log likelihood maximized over $r$ and $p$ given the data. $l_0$ is the log likelihood maximized under the $H_0$ constraint. We use simulation to check the validity of the chi-square approximation. We simulate 500 random variables that are NB(3,0.5) distributed and another 500 random variables with distribution NB(3,0.7). The test statistic and chi-square-based p-values are recorded, and this calculation 
...
We see this phenomonon in the \verb+splatter+ simulation.
 We ran the LRT on the synthetic data, and roughly 1000 genes 
...
but using clustering method \texttt{SC3} in place of the default method.

...
$3/-0.1/1$ to quite weak signals, like $12/0.1/0.4$. The principal components analysis in Supplementary
...
the various data sets presented in Sections 3.1 and 3.2. We first point out that model mis-specification 
...
a within subtype NB assumption at a 5\% false discovery rate (Supplementary Material,
**). Further, for most of these non-NB genes the inference drawn by scDDboost is the same as that
drawn by various other methods.  For example, among 
the cell-cycle genes identified by scDDboost (Figure 1) just one of them fails the NB test. **is
there a fig in Supp?**; also, show latest plot of marginal fits..or maybe later.. }

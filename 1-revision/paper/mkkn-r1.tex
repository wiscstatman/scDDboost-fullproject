...
$3/-0.1/1$ to quite weak signals, like $15/0.1/0.4$. The principal components analysis in Supplementary
Figures S6-S8 provide a view of the global separation between the subtypes.
 We note that the mechanistic sampling model induced by splatter is distinct from the descriptive model 
...
drawn by various other methods (Supplementary Material, Table **).   Among 
the cell-cycle genes identified by scDDboost in the stem-cell example
(Figure 1), just one of them fails the NB test. **is
there a fig in Supp?**; }


\textcolor{red}{The constant-shape assumption is  less well supported empirically, according to 
a likelihood-ratio test that we developed (Supp Fig **).  More than 15\% of genes show 
evidence against constant shape in the examples considered, though inference on differential
distribution is only mildy affected  (Supplementary Table **; Figures **).  **Here's a good
place to add figures showing the marginal fit for some genes**.  Figure~\ref{fig:cdf} shows the marginal fit by mixture NB for transcripts}

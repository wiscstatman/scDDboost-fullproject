...
Figure~\ref{fig:cdf} shows for a random set of called out genes  **CHECK** from the stem-cell study,
...
\caption{Data set GSE64016, random select 9 genes **any of these called bad by diagnostics?**
among genes having at least one positive transcripts, the middle plot (gene ZNF691) is identified violating the constant shape assumption but we can see the marginal fit is good. Here we pool the cells from two conditions. Green curve refers to empirical CDF of the gene transcripts, each light thin pink curve refers empirical CDF fitted on data simulated from our estimated mixture NB model. The dark thick pink curve represents the average estimation. Here we averaged estimations from 10 randomized clusterings. **REDUCE SPACE BETWEEN PANELS AND GENE NAME IN LOWER RIGHT WITHIN**}
\label{fig:cdf}
\end{figure}

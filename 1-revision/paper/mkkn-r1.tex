...
between the two cellular conditions in view. **add a sentence about KS as a suboptimal
choice, but in the same direction of differential distributioon**
...
when using a broad class of within-subtype expression distributions. Subtypes are
considered such 
that cellular condition affects their composition but not the distribution of expression
within a subtype.
...
**change mu's above to component densities, and refer to Supp calculation in 2.1**
...
the final distance matrix is denoted $D=\left( d_{i,j} \right)$. \textcolor{red}{**numerical
experiments..which ones...also used SC3 [cite]**}.
...
\textcolor{red}{add some language to clarify that other methods involved here in comparison}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{} lcccr @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05}| $ & $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05} \cap \mathcalU}_{0.05}|$ & $N_g$   \\
      \midrule
...
\begin{table}[htbp]
  ...
  \end{center}
\end{table}
We also run \texttt{scDDboost} with \textt{sc3} on null case data sets used in Figure~\ref{nullperm}. 
We observe very few positives and conclude that our method does not inflate type-I error with different clustering method.(supplementary 3.4)

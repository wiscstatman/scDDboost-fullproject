...
aims to prioritize genes according to evidence that their expression distribution is different
between the two cellular conditions in view.
...
\subsubsection{number of subtypes $K$}
Inspired by \citep{selK}, we consider a metric of goodness of clustering, defined as \verb validity = $\frac{\textbf{intra}}{\textbf{inter}}$,
...
$\textbf{intra}$ and  $\textbf{inter}$ measure the compactness and separation of clustering. We select $K$ such that $\frac{\textbf{intra}}{\textbf{inter}} < 1$, 
namely the average distance within cluster is smaller than the average distance between clusters. 
...
We observe that for most genes the p values are big across all subtypes. 
...
Let $\mathcal{T}_{0.05}$ be the set of genes identified by two sample t test with fdr correction at 0.05 threshold. 
Let $N_g$ be the number of genes. 
Table \ref{tab:nb} demonstrates that most DD genes identified by our method are also confirmed by t test among those genes where NB distribution may not be appropriate
\begin{table}[htbp]
  \begin{center}
  ...
  \begin{tabular}{@{} lcccr @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      $|\mathcal{D}_{0.05} \cap \mathcal{N}_{0.05}| $ & $|\mathcal{D}_{0.05} \cap \mathcal{N}_{0.05} \cap \mathcal{T}_{0.05}|$ & $N_g$  \\
      \midrule
      EMTAB2805   &593 & 122 & 90 & 45686\\          
      GSE45719   & 528 & 212 & 198 & 45686\\
...
Under those genes against constant shape assumption, Table \ref{tab:cs} demonstrates that most DD genes identified by our method are also confirmed by t test.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{} lcccr @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05}| $ & $|\mathcal{D}_{0.05} \cap \mathcal{R}_{0.05} \cap \mathcal{T}_{0.05}|$ & $N_g$   \\
      \midrule
      GSE75748   & 8605 & 2390 & 2020 & 19097\\          
      GSE45719   & 7454 & 2562 & 2402 & 45686\\
...
Let $p_o$ be the proportions of commonly identified genes among the top 1000.
\begin{table}[htbp]
  \begin{center}
  %\topcaption{Table captions are better up top} % requires the topcapt package

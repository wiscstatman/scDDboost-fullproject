...
Figure~\ref{fig:cdf} shows for a random set of called out genes from the stem-cell study,
  the marginal fit by \texttt{scDDboost}
is still reasonably accurate in most cases.}

\begin{figure}[h]
\includegraphics[width = 0.8\textwidth]{Figs/ecdf.pdf}
\caption{
Empirical cdf for observed counts (green) vs. empirical cdf for estimated counts from mixture NB (pink) using
data set GSE64016. First 2 rows are six genes randomly selected from genes having at least one positive transcripts, 
the last three genes are randomly sampled from non-constant shape genes. Overall, we observe a good approximation. Here we pool the cells from two conditions. Light thin pink curve refers to empirical CDF fitted on data
under one randomized clustering. The dark thick pink curve represents the average estimation over those 10 randomized clustering. **REDUCE SPACE BETWEEN PANELS AND GENE NAME IN LOWER RIGHT WITHIN**}
\label{fig:cdf}
\end{figure}

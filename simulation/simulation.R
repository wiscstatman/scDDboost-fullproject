...
run_de = function(loc,scale,n){
  for(II in 1:n){
    ...
    pDD = PDD(data = data_counts, cd = cd, ncores = 10, K = K, D = D_c,
              sz = sz, hp, pat(K)[[1]], 10, random = T, lambda = 1, nrandom = 50)
    ...
    pDD_sc3 = PDD(data_counts,cd, ncores = 10, K, consen_matrix, sz, hp, pat(K)[[1]],iter = 10, random = T, lambda = 1, nrandom = 50)
    ...
    save(file = paste0("ua/xiuyu/simulation_data/",savename), pDD,pDD_sc3,DD,ED,mast,des,scddres)

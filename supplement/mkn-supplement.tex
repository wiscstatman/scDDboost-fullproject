...
\begin{align*}
&g_1^{g}(X_g^{1,2,3}) = f_0^{g}(X_g^{1,2,3})\\
&g_2^{g}(X_g^{1,2,3}) = f_0^{g}(X_g^{1,2})f_0^{g}(X_g^3)\\
&g_3^{g}(X_g^{1,2,3}) = f_0^{g}(X_g^1)f_0^{g}(X_g^{2,3})\\
&g_4^{g}(X_g^{1,2,3}) = f_0^{g}(X_g^{1,3})f_0^{g}(X_g^2)\\
&g_5^{g}(X_g^{1,2,3}) = f_0^{g}(X_g^1)f_0^{g}(X_g^2)f_0^{g}(X_g^3)\\
\end{align*}


Then the marginal distribution of counts $X_g^{1,2,3}$ is $\overset{5}{\underset{k = 1}{\Sigma}} p_k g_k^{g}(X_g^{1,2,3})$, where proportion parameters $p_k$ satisfying $\overset{5}{\underset{k = 1}{\Sigma}} p_k = 1$ and are estimated by EM algorithm. Thus, the posterior probability of an expression pattern $k$ is obtained by:
\begin{eqnarray*}
\frac{p_k g_k(X_g^{1,2,3})}{\overset{5}{\underset{k = 1}{\Sigma}} p_k g_k^{g}(X_g^{1,2,3})}
\end{eqnarray*}
...
\end{align*}
As prior of $\Phi$ is Dirchlet$[ \beta]$. $n_j = \sum_{b \in \pi} t_b^j}$ for $j = 1,2$
\end{proof}

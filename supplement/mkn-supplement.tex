...
\subsubsection{EBSeq}
In this subsection, we go through how we implement and modified EBSeq to get $P(M_{g,\pi} | X, z)$.
...
$X_{g, s}^I | r_{g,s}, q_g^I \sim NB(r_{g,s}, q_g^I)$,  we made a change to parameter $r$, rather than making it sample specific, we let it be shared at gene level across all samples, i.e. 
...
\end{eqnarray*}
and $\mu_{g,s}^I = \sigma_g(1 - q_g^I) / q_g^I$; For the ease of later deriving the prior predictive function of EB framework we use $q$ rather than $\mu$ to parameterize the NB.  %\sigma_{g,s}^I = r_{g,s}(1 - q_g^I)/ (q_g^I)^2.$
...
We assume marginal density of $X_{g,s}$ is approximately NB with a shared $\sigma_g$ and over-dispersion rate $o_g$ for all subtypes, then we use method of moments to estimate $\sigma_g$ and $o_g$. Namely, we have\\
...
Then the marginal distribution of counts $X_g$ is $\overset{5}{\underset{k = 1}{\Sigma}} p_k h_k^{g}(X_g^{1,2,3})$, 

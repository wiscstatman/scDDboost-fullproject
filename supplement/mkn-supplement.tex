...
$U(\pi) = \Sigma_{b\in\pi}\big[(2c - 1/2) \text{ln}(\beta_b + t_b^1 + t_b^2) 
 ...
 By CLT we know $X_b^1, X_b^2$ are asymptotic gamma($\chi$-square) distributed and $Y_b^1, Y_b^2$ are $o_p(1)$, $g(\xi_b)$ has bounded variance and $T(\pi_1) - U(\pi_2) = -\text{ln}(n)$ if $N(\pi_2) < N(\pi_2)$ so we complete the proof
...
\begin{lemma}
 If $(\phi, \psi) \in A_{\pi_1} \cap A_{\pi_2}$, follow the conditions in theorem 1 and further we have $\alpha^j, j = 1,2$ be vector of integers then 
\begin{eqnarray*}

...
Proof of Theorem 1 [in appendix of main paper]
...
{\bf Randomized clustering}

{\bf Validity Score}  ... selecting $K$
...
**table of null studies + numerical results**
...
\begin{theorem} Following the setting in theorem 1, when parameter $(\phi, \psi)\in Q$,  and we have 
\begin{eqnarray*}
...
\begin{lemma}
For $x \geq 1$, $\frac{x^{x - c}}{e^{-x}} \leq \Gamma(x) \leq \frac{x^{x-1/2}}{e^{-x}}$, where $c = 0.577215...$ is the Euler-Mascheroni.
\end{lemma}
...
\begin{lemma}
 If $(\phi, \psi) \in A_{\pi_1} \cap A_{\pi_2}$, follow the conditions in theorem 4 then 
 \begin{eqnarray*}
...
By Taylor expansion at $x= 1$, $\text{ln}(x + 1) = \text{ln}2 + 1/2(x - 1) - 1/8(x - 1)^2 + g(\xi) (x - 1)^3$, where $g(\xi)$ in the reminder term is $1/3(1+\xi)^3$ for $ 0 < \xi < x$
For a fixed $n_1, n_2$, we have 

\begin{eqnarray*}
\text{RHS of (4)} = (n_1 + n_2) \text{ln}2  - \Sigma_{b\in\pi}(1/8 (X_b^1 + X_b^2)\\
+ g(\xi) (Y_b^1 + Y_b^2) )+ T(\pi)
\end{eqnarray*}
...
and $T(\pi) = (1 - c) \text{ln}(\beta_b + t_b^1 + t_b^2) 
  - 1/2(\text{ln}(1 + \frac{t_b^2 + \gamma_b^2}{t_b^1 + \gamma_b^1}) + \text{ln}(1 + \frac{t_b^1 + \gamma_b^1}{t_b^2 + \gamma_b^2}))$\\
Similarly
\begin{eqnarray*}
\text{RHS of (5)} = (n_1 + n_2) \text{ln}2  -\Sigma_{b\in\pi}(1/8 (X_b^1 + X_b^2)\\
+ g(\xi) (Y_b^1 + Y_b^2)) + U(\pi)
\end{eqnarray*}
  
$U(\pi) = (2c - 1/2) \text{ln}(\beta_b + t_b^1 + t_b^2) 
  - c(\text{ln}(1 + \frac{t_b^2 + \gamma_b^2}{t_b^1 + \gamma_b^1}) + \text{ln}(1 + \frac{t_b^1 + \gamma_b^1}{t_b^2 + \gamma_b^2}))$\\
  
  By CLT we know $X_1, X_2$ are asymptotic gamma($\chi$-square) distributed and $Y_1, Y_2$ are $o_p(1)$ and $T(\pi_1) - U(\pi_2) = O(1)$ if $N(\pi_1) = N(\pi_2)$, $T(\pi_1) - U(\pi_2) = -\text{ln}(n)$ if $N(\pi_2) < N(\pi_2)$ so we complete the proof
...
By lemma 3, $\Sigma_\pi \omega_\pi^{\text post} = 1$ and $P(A_\pi|y,z) = 
\sum_{\tilde \pi \in \Pi} \omega^{\rm post}_{\tilde \pi} \,  1[ {\mbox {\rm $\tilde \pi$ refines $\pi$}} ].$ We get the results of theorem 1 and 2

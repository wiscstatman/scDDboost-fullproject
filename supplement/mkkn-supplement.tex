...
\end{align*}
given the prior $\text{Dirichlet} [ \alpha_b^1]$ of $ \tilde{\phi}_b$ and  $p(T_b^1  | \tilde{\phi}_b)$ is multinomial($\tilde{\phi}_b)$ distributed
\end{proof}
...
Let $V$ denote the orthogonal space of $\phi - \psi$. Notice that $\text{dim}(A_{\pi_1} \cap A_{\pi_2}) = \text{dim}(\phi - \psi) + \text{dim}(\psi) = K - \text{dim}(V)  + K- 1=  2K -  \text{dim}(V)  -1$. 
...
Consequently, there exists $b\in\pi_1$ whose $v_b$ is linear independent with $v_{b'}, \forall b'\in\pi_2$. Thus the dimension of orthogonal space for $\{\phi- \psi: (\phi,\psi)\in A_{\pi_1} \cap A_{\pi_2} \}$ is is at least $N(\pi_2) + 1, \dim(A_{\pi_1} \cap A_{\pi_2}) < \text{dim}(A_{\pi_2})$.
\end{proof}

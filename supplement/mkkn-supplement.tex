...
From empirical study, we observed that \verb+validity+. is monotonic decreasing with $K$
and this trend stabilizes when $K$ is sufficiently big. The stopping rule for searching $K$ is when $\texttt{validity}_{K}  < \epsilon$ is satisfied. 
We set the default value of  $\epsilon$ to be 1, as we found this yields results most consistent with other scRNA-seq methods.

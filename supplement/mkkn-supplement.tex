...
In this section, we illustrate how we estimated parameters for random weighing and demonstrate variability, accuracy and approximation to fully bayesian scheme of generated clustering. 
...
we demonstrate consistency of distributional changes based on \texttt{scDDboost} and Wasserstein distance between the empirical distribution of transcripts.
...
The largest dataset we tried is GSE84465, we randomly sampled 1000 cells from each condition, the reason is we in total have 3500 cells, which take more times to compute if consider all the samples and 
we think 1000 cells each condition would be enough to represent the heterogeneity. We found \texttt{DESeq} identify significant smaller number of positives than others and it is intuitive that we may encounter a lot more subtle changes

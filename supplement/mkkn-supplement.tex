...
We developed a R package \texttt{scDDboost} providing the function to quantify distributional change of scRNA data. 
Details can be found at github site ....
...
We found that random weighting scheme tends to give better results than classical Bayesian clustering. \todo{Fig S2: missing heatmap legend;}
...
We want to have a small intra-cluster distance and a big inter-cluster distance. Consequently we want to minimize \verb+validity+. 
From empirical study, we often observe a monotonically decreasing relationship between the number of clusters and \verb+validity+. 
...
Too big $K$ may end up overfitting such that two subtypes can be very similar. Given that we have a fixed number of samples (cells), allowing more clusters will not only increase the burden of computation 
but decrease the accuracy of our inference. \\
...
There is one thing we need to be cautious, \texttt{scDDboost} may lose FDR control if we keep increasing $K$ (Supplementary Figure~\label{fig:lfdr}).
But from our simulation study, our selecting rule never overestimate $K$. 

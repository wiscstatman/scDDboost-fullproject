...
We developed a R package \texttt{scDDboost} providing clustering of cells, EBSeq and final PDD calculation. 
...
We set the default value of  $\epsilon$ to be 1, as we found this yields good performance in simulation and consistent results with other scRNA-seq methods (e.g. number of DD genes identified).



\subsection{Double Dirichlet Mixture}

In this section, we give proofs for the properties and theorem for DDM in section 2.3 of the main paper.

On the double Dirichlet masses, using notations from the main paper, we have density functions:
...
In this section, we prove theorem 4 and we demonstrate the cases when condition (12) fails. 
...
%We need to study the limiting performance of ratio between $\omega_\pi^{\text{post}}$. Thus we have lemma 5.
...
\sum_{\tilde \pi \in \Pi} \omega^{\rm post}_{\tilde \pi} \,  1[ {\mbox {\rm $\tilde \pi$ refines $\pi$}} ].$  If $(\phi, \psi) \notin \text{Q}$, for all the $A_\pi$ covers $(\phi, \psi)$ there is one finest $\pi^*$ with the largest $N(\pi^*)$ and every other $\pi$ that $(\phi,\psi) \in A_\pi$ is coarser than $\pi^*$. We get the results of theorem 4 by lemma 3.
...
and finally the black dot which is the intersection of those three lines refers to the simplex with finest partitions, $\phi_i = \psi_i, \forall i = 1,..,4$.  When $(\phi,\psi)$ is from the purple line except the black dot, condition (12) would fail. As there is not a finest $\pi^*$ of $H(\phi,\psi)$

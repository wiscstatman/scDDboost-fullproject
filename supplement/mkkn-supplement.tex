...
2. Given condition and subtypes label $y,z$, $(\phi,\psi)$ \perp $X$\\
3. Given $X$ and $z$, $(\mu, \sigma)$ \perp $y$\\
...
$X_{g, s}^I | r_{g,s}, q_g^I \sim NB(r_{g,s}, q_g^I)$. Due to sample-specific size factor in the raw counts, r is made sample-specific. However, since we are dealing with normalized counts rather than raw counts as in \texttt{EBSeq}, we instead make $r$ shared at gene level across all samples, i.e. 
...
We force the size factor to be 1 for all cells and use the same procedure as \texttt{EBSeq} to estimate the failure parameter $\sigma_g$. Namely, we have\\
\begin{enumerate}
...
We compare clustering results between random weighting and Bayesian clustering using the Dirichlet process as a prior in terms of posterior probabilities that two elements belong to the same class given the whole data. 
...
We found that random weighting scheme tends to give better results than classical Bayesian clustering. \todo{Fig S2: missing heatmap legend; labels on scatterplot are vague. Added more detail to legend - please double check it's accurate.}
...
In this section, we give the criterion to select $K$.

To determine the number of clusters, we implement a procedure inspired by \verb+validity+, as defined in \cite{selK}.
...
$\textbf{inter} = mean( || z_i - z_j||^2), i,j = 1,2,...K$, and $z_i$ is the center (medoid) of cluster $i$. 
...
In the original paper $\textbf{inter}$ was defined as minimum distance between medoids \citep{selK}. Here, we instead use the average for the purpose of getting a smoother quantity. 
...
From empirical study, we typically observe a monotonically decreasing relationship between the number of clusters and \verb validity. 
However, this trend stabilizes when $K$ is sufficiently large. The stopping rule for searching $K$ is when $\texttt{validity}_{K}  < \epsilon$ is satisfied. We set the default value of  $\epsilon$ to be 1, as we found this yields DD analysis results most consistent with other scRNAseq methods.



\subsection{Double Dirichlet Mixture}

In this section, we give proofs for the properties and theorem for DDM in section 2.3 of main paper.
...
In this section, we use PCA plots to show the subtle changes underlying each subtype of simulated data and 
we demonstrate consistency of estimated distributional changes based on \texttt{scDDboost} and Wasserstein distance.
Finally, ROC curves illustrate that \textt{scDDboost} has favorable operating characteristics. 
...
Here, the distributional change of transcripts becomes difficult to detect. 
...
\begin{figure}[h!]
  ...
  \caption{first two principal components of transcripts under different parameters for simulated data. Different parameters resulted in different degree of separation of subtypes. We have 4 different settings for hyper-parameters of simulation, each setting has 2 replicates and K = 3}
  \label{fig:pca1}
\end{figure}
...
\caption{Same as Supplementary Figure~\ref{fig:pca1}, but for K = 7}
\label{fig:pca2}
\end{figure}

\begin{figure}[h!]
  \includegraphics[ width = 0.8\textwidth]{Figs/pca12.pdf}
  \caption{Same as Sumpplementary Figure~\ref{fig:pca1}, but for K = 12}
  \label{fig:pca3}
  \end{figure}
...
We also show ROC curves for the simulated data in Supplementary Figure \ref{fig:roc}. here each sub-figure is averaged over two replicates under the same parameters setting. scDDboost tends to outperform other methods .
\begin{figure}[h!]
...
In this section, we provide details of the empirical datasets and also demonstrate consistency to Wasserstein distance on one dataset (FUCCI). \todo{citation?}


\noindent
{\bf Data sets} Details for the datasets used in the empirical studies of the main paper and the estimated number of subtypes $K$ are shown in Supplementary Table \ref{table:1}.
...
The largest dataset we explored is GSE84465, which contains 3500 cells in total. For this dataset, we randomly sampled 1000 cells from each condition, because it takes more time and memory to compute using all the samples. For other datasets we use all the cells within that condition under same batch.
1000 cells each condition seems to be enough to represent the heterogeneity. \todo{How different are results on a different random subset of 1000?}
We found \texttt{DESeq} found significantly smaller numbers of discoveries than others. It is intuitive that we are more likely to encounter subtle changes
...
If the mean expression change among subtypes is incapable to capture the distribution change for some genes, the power of \textsc{scDDboost} will be reduced.
Too big $K$ may end up overfitting such that two subtypes can be very similar. Given that we have a fixed number of samples (cells), allowing more clusters will introduce many patterns (both for mean expression change and proportion change) to infer. Also note the limitation of DDM model (see Section 4). \todo{what limitation is this referring to?} Overestimating $K$ in \textsc{scDDboost} may losing FDR control (Supplementary Figure \ref{fig:lfdr}).

From our empirical experience, 
it is sufficient to capture the heterogeneity underlying cells with number of clusters less than 10 (Supplementary Table\ref{table:1}).  
To demonstrate the change of PDD given different $K$, we present an example using dataset GSE75748. When we increase $K$, the variance of the differential term $\text{PDD}_{K + 1} - \text{PDD}_K$ keeps decreasing and PDD keeps increasing. Our selection criterion (K = 5) happens to choose $K$ such thatchange between $\text{PDD}_{K + 1}$ and $\text{PDD}_K$ is small while not inflating PDD. We generally obtain stable validity score and PDD simultaneously (Supplementary Figure~\ref{fig:rwk}). In addition, the random weighting scheme help us obtain robust PDD (Supplementary Figure~\ref{fig:s10}). Finally, we note that scDDboost may lose FDR control if we keep increasing $K$ (Supplementary Figure~\label{fig:lfdr}).

\begin{figure}[h]
\includegraphics[width = 1\textwidth]{Figs/Kchange.png}
\caption{PDD change under different number of subtypes $K$ for dataset DEC-EC (GSE75748). Our rule for selecting $K$ tends also to make PDD stabilize.4}
\label{fig:rwk}
\end{figure}
...
\caption{PDD under K = 5 vs. K = 6 for dataset DEC-EC (GSE75748). Left panel is without the randomized distance and right panel is with randomized distance. We increase robustness of our methods through random weighting.}
\label{fig:s10}
\end{figure}
...
We have another two lemmas and theorem 1 and 2 are just proposition of the lemma
\begin{lemma}

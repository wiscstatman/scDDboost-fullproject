...
We developed a R package \texttt{scDDboost} providing clustering of cells, EBSeq and final 
DD posterior probabilities. 
...
Through the graphical structure of our model (Figure 3), given
$n_1$ and $n_2$  numbers of cells within each condition,  we 
note that $z^1, z^2$ are multinomial draws, also  given $\phi$ and $\psi$. 
Also given $z$, $X_{g,c}$ is sampled through $\text{NB}(\mu_{g,z_c}, \sigma_g)$, and only depends on 
$(\mu, \sigma)$.
Thus $P(X,y,z | \theta) = P(y, z | \phi,\psi ) P(X | z, \mu, \sigma)$,  
and also  $(\mu,\sigma)$ and $(\phi, \psi)$ are independent a priori given $\pi$.
...
\subsubsection{EBSeq}
Here we recall some key elements from Leng {\em et al.} (2013) on the model behind \texttt{EBSeq},
which we adapt to get $P(M_{g,\pi} | X, z)$.  Suppose we have $K$ subtypes, let $X_g^I = X_{g, 1}^I, \cdots , X_{g, S_1}^I$ 
denote transcripts at gene $g$ from subtype $I, I = 1, \cdots, K$.  The \texttt{EBSeq} model assumes that counts within subtype $I$ are distributed as Negative Binomial:
...
Following Leng {\em et al}, we assume a prior distribution on $q_g^I : q_g^I | \alpha, \beta^{g} \sim Beta(\alpha, \beta^{g}).$ The hyperparameter $\alpha$ is shared by the whole genome and $\beta^{g}$ is gene-specific.  We force the size factor to be 1 for all cells and use the same procedure as \texttt{EBSeq} to estimate the shape parameter $\sigma_g$. Namely, we have
\begin{enumerate}
...
the counts from these groups are essentially pooled: 
 ...
 $f(X_g^{I,J}) = \int_0 ^1 P(X_g^{I,J} | r_{g}, q_g) * P(q_g | \alpha, \beta^{g}) dq_g = \Big[ \overset{S}{\underset{s = 1}{\prod}} {X_{g,s} + \sigma_{g} - 1 \choose X_{g,s}} \Big] \frac{Beta(\alpha + \Sigma_{s = 1}^S \sigma_{g}, \beta^{g} + \Sigma_{s = 1}^S X_{g,s})}{Beta(\alpha, \beta^{g})}$.  Consequently, the prior predictive function for $P1, \cdots , P5$ is
...
Then the 
marginal distribution of count vector $X_g$ is 
...
In this section, we review and extend Dahl's modal clustering procedure  (Dahl 2009).
...
Let $X = (X_1, X_2, ...,X_n)$ be a vector of observed data (say at one gene). 
Given a partition for the data $\pi = \{S_1, \cdots, S_q\}$, where $S_i$ are disjoint subsets 
of $\{1,2,\cdots,n\}$ and $\bigcup_{i = 1}^{q} S_i = \{1,2,\cdots,n\}$, the probability 
for $X$ satisfying such partition is
\begin{eqnarray*}
...
The prior distribution for a partition $\pi$  also takes a product form. We aim to compute
...
We plot the ARI (adjusted RAND index) between the randomly generated clustering to clustering under the original distance across eight datasets. The boxplots indicate that random weighting is inducing
...
clustering (Jara et al. 2011)  on  a simulated dataset. 
...
When $\phi$ and $\psi$ satisfy only the coarsest constrants: 
...
we have $E_{\tilde{\phi}_b}(\tilde{\phi}_k) =  \frac{ \alpha^1_k }{ \sum_{k' \in b} \alpha^1_{k'} } $ and $E_{\Phi}(\Phi_b) = \frac{ \beta_b }{ \sum_{b' \in \pi} \beta_{b'} } $. The case for $E_{\pi}(\psi_k)$ 
is similar.
\end{proof}
...
Let $V$ denote the orthogonal space of $\phi - \psi$, when $(\phi,\psi)\in A_{\pi_1} \cap A_{\pi_2}$, and $\text{dim}(A_{\pi_1} \cap A_{\pi_2}) = \text{dim}(\phi - \psi) + \text{dim}(\psi) = 2K - \text{dim}(V) - 1$. Also let $\pi_1 = \{b_1^1,\cdots,b_s^1\}$, and  $\pi_2 = \{b_1^2,\cdots,b_t^2\}$. 
The corresponding vectors are $v_1^1,\cdots,v_s^1$ and $v_1^2,\cdots,v_t^2$. 
We claim there must be a $b_i^1\in \pi$ whose corresponding $v_i^1$ is linear independent 
...
For some $\pi$, $P(A_\pi, | y, z) = \underset{\tilde \pi \in \Pi}{\sum} \int_{A_{\pi}} \omega_{\tilde \pi}^{\text{post}} d\phi d\psi$, 
...
By Bayes's rule we know $p(\phi,\psi | y,z) \propto p(\phi,\psi, y, z) = \sum_{\pi \in \Pi}  p(y, z | \phi,\psi) p_\pi(\phi,\psi)\omega_\pi$
...
\begin{figure}[h!]
  ...
  Different parameters resulted in different degree of separation of subtypes. We have 4 different settings for hyper-parameters of simulation, each setting has 2 replicates.
...
ROC curves for the simulated data in Supplementary Figure \ref{fig:roc}. Each sub-figure is averaged over two replicates under the same parameters setting. \texttt{scDDboost} tends to outperform other methods .
\begin{figure}[h!]
  \includegraphics[ width = 0.8\textwidth]{Figs/Roc_supp.pdf}
  \caption{Roc curve of the 12 simulation settings, under each setting, TPR and FPR are averaged over two replicates, generally \texttt{scDDboost} performs better than other methods}
  \label{fig:roc}
\end{figure}
...
But in our simulation study, we note that the validity score method was always conservative, and did
...
In this section, we prove Theorem~4 and we discuss cases when condition (12) fails.

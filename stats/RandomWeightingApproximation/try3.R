...
# like try1, but add random weights to squared distance
...
for( b in 1:B )
 {
  print(b)
#  e <- 4*lam*rgamma(n,shape=1)  ##  cor 0.98
#  e <- 8*lam*rgamma(n,shape=1/2) ## cor 0.96
#  e <- 8*lam2*rgamma(n,shape=1/2) ## cor 0.88
#   e <- 4*lam2*rgamma(n,shape=1/2) ## cor 0.90
#   e <- 4*lam2*rgamma(n,shape=1) ## cor  0.82
#   e <- 2*lam2*rgamma(n,shape=1) ## cor  0.92
#   e <- 6*lam*rgamma(n,shape=(1/2)) ## cor  0.95
#   e <- 10*lam*rgamma(n,shape=(1/2)) ## cor  0.97
#   e <- 12*lam*rgamma(n,shape=(1/2)) ## cor  0.98
#   e <- 20*lam*rgamma(n,shape=(1/2)) ## cor  0.93
#   e <- 16*lam*rgamma(n,shape=(1/2)) ## cor  0.96
#  e <- 8*lam*rgamma(n,shape=1)  ##  cor 0.965
#  e <- 6*lam*rgamma(n,shape=1)  ##  cor 0.981
#   e <- 3*lam*rgamma(n,shape=1)  ##  cor 0.97
#   e <- 5*lam*rgamma(n,shape=1)  ##  cor 0.982 [tau1=tau2=1
#   e <- 5*lam*rgamma(n,shape=1)  ##  cor 0.983 [tau1=tau2=1/10]
   e <- 12*lam*rgamma(n,shape=(1/2)) ## cor 0.980     [tau1=tau2=1/10]
  bar <- dst.m^2 + outer(e,e,"+")
  dst.star <- as.dist((bar)^(1/2) )
